...
视频播放结束后，应该恢复进度条和当前播放时间为初始位置，并将播放按钮状态设置为暂停状态。
...
当前播放时间以及进度条可以随着视频播放实时更新。接下来有一个点，我认为不太合理。例如视频播放完后，我认为这个视频特别有意思，想点击播放让它再播放一次，但是点击后没有任何反应。同志们注意看，接下来我们需要让它支持多次播放，播放完成后再点击播放，仍然需要能够播放。播放时加一个分割线，改成黑色的拿过来。当视频播放结束后，我们需要点击播放按钮或者重新点击播放按钮后视频能够重新播放。最贵的视频可以重新播放。另外，我们可以将S gart中的starting方法重新调用一次。Starting方法的内容是将视频加载完成后重新播放，找到黑芝普莱尔类。我们在这个领域之前为他实现了安普勒的playing方法，我记得有一个对start playing的方法，让他进来之后，这一步直接调整他的start是否亮起。Start play在内部会直接加载视频，因此我们将来可以重新调整这个方法。当我们在里面点击播放按钮时，需要注意，了解视频是否需要重新播放，还是进入的位置。我们在播放完视频之后需要将图标修改，播放状态也需要修改。播放完视频之后应该停止，不应该是play的状态，应该将其改成force。我们可以通过播放时间检测视频是否播放完。如果需要重新播放视频，我们确实需要将它调整一次。我们需要对这个位置进行调整。我们刚才提到play time，如果这个time等于vidov6的totl突突，那么视频播放时间在我们这个类当中，它这里面有10，那么说明视频播放完成之后，我们需要进行修改。我们在里面给的a是福利的变量，将其修改为暂停。除了修改之外，我们还需要修改播放按钮上的图标，将图标设置为暂停，这是一种三星的功能。如果我们将来需要做这些事情，那么只能在阿姆普勒位置改变的方法中检测。我们只有在这个方法中才能获取播放时间，并在其中进行福利和检测。即在这个方法起立并更新完成之后，如果我发现你的播放时间在里面是10，那么说明这个服务类非常麻烦。如果Play time在里面等于10，即我们的视频总时间，说明播放完成。我们将is Play改为force，然后UI我们需要去site spell shit。请设置它的样式。这是borbl包的杠mhe包的emerge，URL在我们当前目录下有一个a max目录，在这个目录下有一个pmay辅类配置目录，在这个目录下有一张暂停图片将它们修改过来，价格为25万元。我们主要支持的是视频播放完成之后，将播放按钮到按钮的图标以及状态设置为暂停。如果用户想要播放，那么他可以点击播放按钮。在这个位置，我们可以重新点击播放按钮。如果你想播放视频，那么我需要了解它目前处于重新播放状态，并检测它重新播放的位置。我们的PLA是否需要通过普雷泰姆与之前的10进行比对，如果它等于10，那么说明你确实需要播放。A的play asplay是播放状态，我会让它播放。你每次进来播放完之后，我们将as play设成force，设置force之后你再点播放，他进来取上一个反射，反射又重复，这样就让视频播放起来了。当你需要播放时，我们需要重新调整Start的方法，你去加载这个视频，将斯达特普利银行进行调整。如果你需要调整Start play，我们就需要了解视频的路径。视频路径和播放时间，我们需要定义两个常用变量，一个是int64当t，另一个是pray服务类time，最初将其改为0，它可以记录当前视频的播放时间。Q4证，他拿的vid有video pass，我们必须保存视频路径，在这个位置将Vido video pass传入。你进入系统后需要寻找此方法，此方法最初在video boss中调整。我们在video box中调整之后将视频路径传入系统，并且在播放之前保存视频路径。后续重新播放时需要使用该路径，我们先将路径保存。This vid有，我们刚才给他确定了视频路径，然后重新赋上值vido僵尸路径。之后还有播放时间。在你重新播放时，这个位置需要清零，当然不清零也可以。下次再播放时，我们获取的时间是0。这一步我们需要为弗雷特附上一个值，这是pmay弗雷特。如果你不复制，它里面永远都是0。福利太好，这个过程大在里面给他梳理一下。我将代码拿来放到板书中查看。点击播放按钮，将其放在这个位置。你也拿过来，首先这个视频在不断播放，当视频播放完成后，我检测发现你的播放时间已经等于视频的总时间，我们就让它停止播放。你无法播放，将图片修改为暂停状态之后，我想点击这个按钮让它重新播放。你将艾滋福利加入后，将图片修改为播放。我发现他现在确实需要播放，你将star的方法去掉，让它重新加载视频，在里面播放即可。我们在其中大致给出一个逻辑，实际上这方面并不复杂。为了让程序运行起来观察效果，我们让它编译一起。目前正在编译，快程序运行起来后，我们随便点击一个封面，然后将视频播放一次。我们发现你的当前播放时间恰好与视频的总时长相等。我们是否将播放按钮的图标设置为暂停？暂停了，现在的播放状态也是force。当我们再点击播放按钮时，问题在于它会将播放状态设置为触。所以当它继续前行时，我们应该检查播放按钮的图标是否已经变成播放。这个位置再点播放已经没有意义，因为视频已经播放完毕，所以它继续前行会走到底部的f中。我们在进入底部的f中逐一检测，发现时间位于这个位置，里面是10，这说明你需要重新播放。并且它也是一个播放状态，我们在里面把视频播放起来。发现虽然视频加载了，但是没有播放。再点击它，现在又成为暂停状态。接下来我们点击播放按钮。我发现了视频。它在里面才播放，这说明我们刚刚所写的大逻辑上没有问题，只是加载了之后确实加载了，但是没有播放。没有播放的原因是我们之前让视频加载了之后，这一部分将它改成了暂停状态。因此我们start to playing完成之后，需要让它重新回到播放状态，也就是说底下最贵的我们给它上MPV plan，MP plan，然后让它重新PLA完之后播放。你可以将这个点放入f中，让他重复判断。你是播放状态，但是播放时间是0，重新加载完后你再按播放进行处理。既然已经是播放状态，这个位置就不需要检测。如果你发现你是重播，我们先让它加载，加载完后把视频播放起来，放这个位置进行处理，这样可以减少重复操作。我们可以将其放置到底部进行处理。接下来让视频先通过程序运行，观察我们刚才在添加播放后，再进行重播时它能否实现。这个视频正在我们的程序化中编译，只能耐心等待。程序运行后，先让它播放一遍。图标已经回来了吗？点击时需要注意观察三个点，首先是视频是否已播放。第二当前时间位置和进度条是否同步更新？我们继续前行。我们在里面支持重复播放的功能，整个时间同步位置就可以处理完成。
