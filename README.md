...
QString QFileDialog
:getOpenFileName(QWidget *parent =nullptr, const QString &caption =QString(), const QString & [
[static]
静态方法
dir = QString(, const Qstring &filter = QString(), QString*selectedFilter = nullptr, QFileDialog:Options options= Options()
...
老师课上的讲话：“
接下来我们给修改图像的按钮，把槽函数去绑定一下，让用户能够支持图像的修改。好，回到我们的满外地拖 cpf4可以到这个图文件当中，我们先给它定义上一个陈述对吧？陈述这个是胃癌偏方癌胃癌记忆，然后SL不如TS好，完了之后让他往下来走的话，不能说报一套是我们到输给用户头像对吧？那是avatpr顶天cmscpd修改图用户图像按钮，把这个按钮在里面给出来了之后，然后加回车在原文件当中生成，对吧？说明它的定义。好再往下来走，那么也得需要去绑定它的信号槽了，对吧？绑定它的信号槽绑定信号槽了，我就把它放到我们的初始化方法当中，这个人给一个connect可耐草，然后是我们的uiui aava这个是aava这样，然后他将来会出发一个q扑车，8b尤其结婚对吧？听58层它有一个点击的信号，这个信号我让当前满y的页面当中的 P值基本上是满y的，对吧？他那边有一个维特一千克莱克这个槽函数来进行处理，已经处理好信号槽给它绑定了之后，将来我们在这个槽函数当中所要做的事情，一第一弹出打开对话框对吧？选择用选择用户头像，或者说从本地磁盘这个位置上它对吧？弹出本地对话框，将来我们必须要用到评估范围，已经做类当中的就是该图范围，你跟他在我带了对话框了对吧？好，也就是说将来在 Q贷款qfi一KO目标运作在这个类当中，它有一个盖特欧朋三内部的一个方法，那个方法他就可以帮到才对方方能打出文件对话框的刘华刚显示出来对吧？显示出来ok完了之后然后再让它往下走，我们的二二第二步我们就得需要因为他将来给我们返回的是一个就是你选择的图片的路径对吧？路径我们得需要打开获取图片的数据或者这样的数据再往下走到三第三个步骤，我们将这个图片数据这个是图片的这些数据把它设置成我们所给的按钮的图标就可以了，对吧？把它设置成按钮的图标。Ok，根据我们现在所给的这三个步骤，我们一个的来进行处理，首先我们得需要用到就是q三和代码的当中的，但是ok范围的方法对吧？这个方法 ok然后把这个文档打开，我刚才已经把这个方法已经查过了，对吧？查过了好把它拿过来，这个方法它是我们qt，这个方法它是我们 Qq发要带了对吧？大家都带了，这是一个内容，这个类当中的一个静态的方法，可能它后面带了一个study，但是他可能提高，那就说明它是我们类当中的静态方法方式错了，静态方法静态方法，我们将来可以直接通过类名加作用域的方式来进行访问，对吧？访问。它在里面有这样子的几个参数，第一个参数就是设置它的副对象，设置父母对象对吧？设置非常是父母对象，但如果要把它改成空的情况下，它就没有副对象对吧？然后这个位置上它怎么去设置窗口标签对吧？我们到时候可以把它设置为修改图像这个图像。再往下走到第三个它也是一个字符串，这个字符串的名称给的是一个dir对吧？Dir我们的窗口显示出来之后，窗口显示出来后的默认目录默认情况下，我们可以打开哪一个目录对吧？打开哪个目录再往下走，这个是我们所给的过滤串。因为一个目录底下它的文件可能比较多，比如说所以我们给它可以设置一些就是过滤串，过滤串或者说把它是当成一个数据的过滤器，比方说我们6层的这种方式，我们相当于将来在里面你把当前的文件夹底下的就是图片PNG的spm，这个图片就能显示出来对吧？然后他这个文章当中都已经有了一个用意了，那么在里面可以直接把用意给他拿过来，注意，刚才忘说了它是有一个返回值的，返回值也是自创型的，我们看到它最终我返回的内容就是我们刚刚选择的字符串，它所对应的路径对吧？锁定的路径好了，我们自己还是在代码当中去敲一下，如果用的话我们先得需要包含文件，这个是井号一一刻录的，井号一刻度的，然后 Q带o引爆对吧？百五包含进来，包含进来了之后然后来上QS，这地方是我们给他翻屋pass没办法，p截止还有pass，完了之后丢fan，你做好以后那地方它有一个概括，东风怎么出来了，最主要是第二层都跟我们同价包含对了吗？我看一下。Q815以后，他说q三以后没问题，我看一下 qfio以后出不来的话，那就说明它不太对吧？我试一下q当代劳动d法a我一定平分带路口。好在这个过程中他来上一个比如five第二楼的对方房贷，第二楼到他那地方有一个盖车，在屋内对吧？部分范围内ok这个咱们刚刚在那写错了就把它放出来。然后其实它前面这一步应该是文档当中已经有说明了QQ带有代号了，对吧？好，回答了之后，按照我们刚才所说的第一个他的服务对象对吧？服务对象这个服务元素我们就不给了，就把它设置成一个空，然后再往下走到第二个参数就是它的窗口标题，窗口标签我们把它给修改，基本上是修改图像对吧？这个图像。第三个是它的默认的目录的话我也不给了，待会可以看一下它的目录。然后再往下走，就是它在里边给的过滤串，过滤串，我们可以直接用它给的实例当中的过滤串，对吧？把它拿过来，拿过来了之后，整个位置在显示的时候，我将来只需要让它去显示图片就可以了，对吧？这两张类型的图片。Ok这样子的话我们就把这个图片拿到对吧？拿到了之后按理来说接下来我们应该去的获取图像数据了，但是在获取的时候，用户他把对方框打开，他可能没有选就直接点击取消对吧？点取消，所以这个位置上我可以先点检测一下，检测一下5o plus分5plus它是不是空的，因此安普如果它 Fire pass，它在里面是空的情况下，那么这个代码就直接反馈说明用户他对吧？没有选择图片。如果不空这个规则，我们先把它去打印一下三o Pass把分化把它打印一下，看一下待会我们图像的路径对不对，对吧？对不对？完了之后刚刚操盘手我们在里面已经对它进行了绑定，对吧？接下来让我们的程序运行起来，我们看一下，待会点击了用户头像之后，我们刚刚所添加的获取打开文件对话框，它是否能弹出来，而现在这个程序正在编译。当然第三步之后，它其实还有一步，还有一个就是你把头像设置了，我们将来肯定得需要把用户头像给它上传到服务器里面去。Ok那进来了之后点击能看到这个对话框确实在里面弹出来了，对吧？弹出来了，然后让他往回被保人这一步，第一个有一个m值和m值，myself他比方说把他们所有的用户图像打开对吧？打开了之后他们在底下确实看得到，这个它在里面已经将我们所给的用户图像的路径已经打印出来了，对吧？用户的路径已经打印出来了，用图像的路径打印出来了之后，再往下来走，我们就得需要给它去加载本地的图片了，对吧？下载本地的图像加载本地图片，它实际上就是让它在档期我们去读文件，读文件读文件的话，文件的操作加载判决的图，文件的操作，我们将来在其他位置也得需要用到，所以说读取文件的方法，我们将来就把它封装好，封装好了之后放到我们的YouTube当中对吧？将来在这个位置上我们再给它封装两个方法，一个读取文件的操作对吧？读取文件的操作，一个当初是系列文件操作，写文件操作，首先要操作，读取文件操作，我们从指定文件当中获取它的二进制数据，我们把这个文件以二进制的数据都取出来。接下来我们将读写文件操作的两个方法优化封装。好，ok，我们先来看一下就是读取文件的一个操作，或者说把它称之为读文件的操作，读文件这个操作它是从指定文件中基本上是读取所有的二进制内容，然后将其保存到q阿q，拜托老瑞中就可以对吧？怎么对？好 Hgst提高英s的为什么加三的话，我在前面写该命的时候已经说过了，三可能就是为了防止这个图文件包含多次的这种方法出现多次定义你来就是让它因为我们的方法都比较短，可以把它当成内联函数来进行处理。故障它先是读取文件的操作对吧？读取文件操作读取文件的操作故障就把它改成load file，load file就是加载文件对吧？或者说你也可以把它改成一个window file都行。大概意思宝贝您说我们把这个文件数据给它放在一个best当中了，参加出来就把它给烫死了，q斯顿就是准故障有一个就是file pass，你把我路径给我传一下就可以了，对吧？传进来之后将来再进行文件读取的时候，我们可以利用koj可能提供的k05的这个类来进行文件的读取，ok可以看一下这个是k05，Q5的类它专门是用来进行文件读取到对吧？进来之后你可以先构造个对象，把文件的路径给进来了之后，然后我们还得需要把文件给它打开，底下是有一个打开的方法，打开的时候我们可以给它去设置你打开的这种模式，对吧？这种模式。这个打开看一下，打开的模式它在这边有我们只读的方式打开对吧？只读的方式打开，然后故障就是以执行的方式给它打开，怎么都行对吧？我们要去读文件。好，然后再往下来走，那就是你去读取文件里面的内容对吧？然后往后可以读取文件内容，读取文件内容的话我们要给它进行瑞德，你在里面去找一下，它里面不是应该肯定是有对吧？瑞德的方法来看一下，让他往下来找网上来找姨在这场还没有看到味道，艾瑞德味道一规定如果没有的话，就看他有没有积累对吧？他说继承的 Qq范围代码的融资它里面你再去找他应该是有味道范围对吧？味道对他遇到烂地方等等，或者说我还可以怎么看，在这个它有列出所有的成员，列出所有的成员，就是把他自己内部添加的成员，以及他私立当中的一些继承下来的成员全部给他罗列出来了对吧？还有有味道，就是一次性把文件当中的这一些所有的数据给它全部都到对吧？读取渠道。读取完了之后，我们的文件操作已经结束了，最后别忘了把文件拿给人家关闭掉就可以了，对吧？关闭掉。好，接下来进来之后，我们通过 Q q范围限制它构造一个文件对象，还是我们的file pass，再往下来走的话，我们把文件给它打开，对吧？打开不管类型， ok这个是file，我给他点上一个open，ok文件打开的时候，它在这个位置有打开的方式，我们给一个q q在 q范围内的内部，它有一个reader， reader only你们都说我们以只读的方式将文件打开，那打开你到底有没有打开呢？那就是把这个ok检测一下，如果打开成功，人家就反复的输入，打开失败它返回去boss，这一点其实和c语言当中我们的那个f都可能有点像对吧？再往下来走了，如果你打开失败了，这一步让我就算来上一个落个重复一下对吧？文件打开失败，换成他的失败，那文件既然已经打开失败了，我们就给他直接在这个给他返回就可以了，对吧？返回实际上在里面给他返回，返回当然在返回的时候，刚刚我们这边的话忘了他设置返回值了，返回的时候你将不让人说可以玩，你将来把读取到的内容就是把它以字节数组的方式就能返回出来，对吧？所以这个味道我给你返回的空的，他反馈辅导，然后再让他往下来走了，底下就是q拜托格瑞 Q拜托a对吧？拜托瑞，这个就是咱们里面的文件数据了，没办法大家可以就直接给他或者说CN去研究文件当中的内容，这有他那边有一个瑞德奥容易骄傲的一个方法，一次性就将文件当中的这一些内容让它读取出来了，读取完了之后，债务点到CF克鲁斯把文件给人家关闭掉，最后每份对吧？你说c问题你把它记住，把它填进去，这样子的话我们就将文件就是指定路径当中的一些文件授权给它读取，读取到我们程序当中，好写文件操作。写文件操作这一块我们是将 q拜托尔瑞对吧？刚好我们的读选在里面是反了，将 q费用艾特尔瑞中的内容写到指定的文件中，我们把它写到指定的文件里面去，所以说 static自带这个是一这一次我们就不需要把它去设置反馈去了，因为你写完就可以了。然后WR it right范或者说right外套对吧？恶锐然后黑板突犯，ok同样是错的，还有此证非要问这个老师，发o怕死，你把为文件或者说这个地方还有密码都行，你把文件的名字提供这样对吧？情况下这个味道，你就给个名字文件名提供给老板之后，然后 com3q拜托瑞a a完，后面跟着就是我们给到文件的内容对吧？C ng d ng，我们同样给它加上一个引用，因为 q掰头因为它也是个对象对吧？ Ok。然后在底下把这个方法定义给出来，上面这个没有的话需要换一下，发现这个是文件名，对吧？好，接下来如果同样也是给他去创 qfil的对象 QFII我想你把这个费用内码的话传进来，传进来了之后，然后不管类型我们给一个ok对吧？Ok那么等于等于f点的都很大都很第三，那就是我们的q范围这个类当中，它有一个刚刚才你在上面是只读对吧？只读现在我们就给他来上一个right认同。Right，分类对不对？有一个分类，现在我们是以执行的方式把文件在里面就是打开对吧？打开完了之后 if如果让 ok它这里面是否是您说明我们打开文件失败，这个问题的打开文件它怎么就失败了对吧？是。Ok打出玩家失败，我们就直接对再往下走，接下来我们所需要做的操作就是非欧点的乱走非欧点的地方是writ乱跳，后来他把那个数据量给他写进去，对吧？Contentent考证，他写到文件当中，写到文件里面之后，接下来我们再给他来再来上一个 flash，从20~90，把文件缓冲区当中的数据量它全部写到文件里面去了，因为这个费用它写的时候不是说我直接写文件，它是先写到缓存对吧？缓存最后范围点的cl和SC把文件分享给他关闭了。Ok这个的话是我们在里面给他给到两个楼距，这样操作对吧？给完了之后再回到我们的外地说点CPP当中，刚才你的第一步是已经处理完了，第一步处理完了，接下来我们所要做的事情就是从文件当中给它读取数据对吧？读取数据的话，这一步就是加载和加载路，德范文章图，白特瑞，你把文件这个是我们刚刚拿到的文件pass对吧？文件路径，那就把它换成文件名字对吧？你怎么叫都可以对吧？这个位置你直接给成非o pass，它也没什么问题，ok路径文件pass文件名。好在里面让他去读，他读完了之后将来是以二进制的方式显示出来的，而以q best方对象返回来的，所以前面我们采用q bet olive接收一下对吧？接收一下，这个就是我们的读取到的文件当中的数据，对吧？文件当中的数据，当然再往下走了，你在读取到了之后，这块我们还得需要最好判断一下，你读取到内容是空的，因为它有可能会返到你的文件打开失败了，它可能会给我们返回一个空的，能下来的话会给我们返回一个空。好，也就是说非法还有非常，如果他在里面是a字m1MP7y等于是安排如果它是通，那就说明我们在这个位置上就是读取文件发现失败了，对吧？底下也没办法操作了，直接让它返回就可以了，返回好上面这一位置上也给它加上这个信息，如果他返回的文件路径上是空的，那就说明用户在这个位置上他就取消对吧？取消选择头像，我讲他根本就没有选对吧？在里面没有选，ok那文件数据有了之后再往下走了，我们接下来就得需要把文件数据量给他，甚至导流式的都做完了，你修改文件按你的头像上去了对吧？图标上去了，他有个塞车a卡 X的对象，然后塞车a卡它需要一个塞车a卡，对吧？方法。这个工作方法它的里面需要我看一下，它在这个位置上需要的是一个皮克松麦克对吧？皮克松麦克它里面的的就是参数还比较多，然后我看一下再没有其他的我们在干脆这样，咱们再给他一做上一步的中转，就这个我们把现在你拿的是文件的数据，我们把这个文件的数据直接给它转化成一个就是q艾特对吧？ Q艾特然后转化成q艾特，我们在这里看一下， q IC我们q艾特q艾特它的内容就是现在我们拿到的数据对吧？拿走的是文件的数据，它有一个QQ搜MAC的方法 PK搜MAC，有基因可从外部 QQ从外部到它内部，有一个方法一个方法就直接可以把一个文件数据给我，转化成一个就是图标，我看一下。它内部有一个就是漏的路德服装对他对吧？你提供一个贝尔卡尔瑞，我们先把图像给它添加进来，添加进来之后现在就相当于我们的数据就把它变成了q Picazo的一个对象，mak的对象对吧？ Qq卖房我们可以直接用它去创建，qx应换成qx好，接下来你看一下具体我怎么来实现就可以了，对吧？怎么来实现？Ok，同样我们把这个方法也作为一个工具方法，作为一个工具方法把它放到我们的每一颗 Youtube当中对吧？底下咱们可来上stat SS单体课这个是in life对吧？In life，然后它将来反馈的是一个qi卡 Qh然后米克艾特米克艾克这个是q艾克尔瑞，上次说q二x二a22a完了q32a然后它是我们的图片数据，Conte文件对吧？或者说是我们的就是magemSD卡就是我们的头像数据，ok处理完了之后，接下来在他内容我们先给他创建一个q皮克松map，没有让我们把所有的文件包含起来，这一部分已经透露的，然后它是qpe和so map对吧？ Map然后紧靠英特路的，我们这一块包含qit两个同价给它全部包含，完了之后再让它往下走，这个位置上就给我p一个说对吧？ Pe格搜，好你将你的mad把你的文件数据直接给我传进来，对吧？传进来，传进来了之后，然后我看一下他们内部有这个方法吗？Pos map poc MAC构造方法它还没有对吧？没有了我们就直接通过它的加载来加载，然后这个位置有加载的方法，ok这块就先把对象创建好，对象创建好了之后，然后让 Texo map你不是有路德斯郎德好对吧？他们可以这意思，你把文件数据它加起来，加载进来了之后，接下来我们再给他来上个q对吧？对IC我们然后结果用皮克松把AP rap了，你直接去进行创建就可以了，完了之后将 iPad膜返回来，这可能是IC欧根对吧？Som。好完了之后回到我们的因为y里头点CPP当中，那么这个位置我们就直接调用刚刚我们所写的每一颗艾特对吧？每个艾特你刚才看然后将这个分数低传给它传递进去就可以了，把它传递进去，这样子的话我们就把文件图像给它设置了，设置完了之后第四步为这个位置它还没完对吧？那么还需要将服务添加数据就是上传到服务器这一步，我们等到后续再来进行操作，对吧？再来进行操作，来接下来我们让程序运行起来看一下，它将来处理完了之后，我们能否能把用户图像给它修改掉，代码它现在正在编译，不行。好程序运行起来之后，我的对吧？然后点击修改用户头像，让他往回退一步再退一步。对，那只是底下有一个满赛尔玛的时候，我给他找一个用户头像，打开对吧？打开这样子的话，我们的用户头像他就上去了对吧？上去了，虽然说用户头像上来了，但是他头像他感觉到是非常小的，对吧？这两个比较小，比较小，我看一下刚才在我们的UI文件当中，这个是满足在5点UI对吧？满足5点UI然后让它往下来走，我看一下，我们在底下它的 I iPhone赛的话，我们已经把它设置了，既然把它设置了，还是我们的图像的问题，对吧？因为这个图像它本来可能就比较小，所以说加载上来的图像我们还得需要再进行进一步的处理。”