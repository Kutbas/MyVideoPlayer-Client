...
拖拽一个QLabel和QComoBox到kindContent中左右摆放，objectName从左往右依次修改为
kindLabel和kinds，再拖一个水平弹簧放在kinds之后，然后选中kindContent点击水平布局，将
kindContent的LeftMargin设置为16，其余Margin均设置为0，Spacing设置为30，将kinds的
minimumSize和maximumSize的宽高修改为320*49，placeholderText修改为"选择视频描述"，将
kindLabel的文本设置为"分类"
拖拽一个QLabel和QWidget到tagContent中左右摆放，objectName从左往右依次修改为tagLabel
和tagWidget，tagLabel的文本修改为"标签"，tagWidget的背景色修改为rgb(0,255,127)，然后选中
tagContent点击水平布局，将tagContent的LeftMargin设置为16，BottomMargin修改为20，其余
Margin均设置为0，Spacing设置为30,将tagLabel的minimumSize和maximumSize的宽修改为32;
拖拽一个水平布局器QHBoxLayout到tagWidget中，objectName修改为tagLayout，然后选中
tagWidget点击水平布局，将tagWidget的Margin和Spacing均设置为0
拖拽一个QPushButton到btnWidget，objectName修改为commitBtn，文本修改为"提交"，然后
拖拽两个水平弹簧，放在commitBtn的左右两侧，选中btnWidget点击水平布局，将btnWidget的
TopMargin设置为10,BottomMargin设置为34，其余Margin和Spacing设置为0，将commitBtn的
minimumSize和maximumSize的宽高修改为136*48
...
老师课上的讲话：“
接下来我们将剩余的几个步骤给它处理完。刚才我们处理的是简介部分，简介部分完了之后，现在还剩余分类标签以及最后的提交按钮，中间的分割线将来我们通过样式来进行控制就可以了。好，再继续往下来走，他下一个他说拖拽一个q利宝和q com boss的照看得肯特的钟左右摆放，这一侧要给它左右摆放，那么先找到看得看得到。Okay，回到QQ当中，然后把底下的水平滚动条往右拖，拖完了之后将垂直滚动条往下拉对吧？然后再把它往左拖，先找到我们的刊登content，我说我点一下看一下右侧确实是刊登content，那么这次他说了让我们拖拽一个q利宝和q外地通道，做上面这个位置q利宝和q com boss对吧？来我们先拖拽一个q利宝，q利宝给他拖进来，拖进来了之后，然后还得需要拖拽一个康boss。好找一下，在右侧左侧找一下就康boss，这个空间就类似于我们上面的，你点一下它里边添加的这些项目就可以把它列出来，ok拖拽进来了之后然后水平摆放对吧？包括家庭内部从左往右依次修改为刊得利宝和汉字，来我们你把它修改为刊登利宝，刊登对吧？分类的一个标签，然后将 Combo可能把它修改为看字，因为它内部将来得需要包很多的分类，完成了之后再拖拽一个水平弹簧放在看样子之后，然后点击看样子讨论了进行水平布局。好，我们先给他拖拽一个水平弹簧放在，当boss了之后完成了之后选中看到可分了，然后来上一个水平布局，水平布局结束了之后，我们应该还得需要修改它的马丁对吧？将来妈只能修改为16，其余吗这行均修改为0，CBC行修改为30，好，然后选中将右侧的属性补充条往下拖net给它修改为16，其余8级全部修改成0000，c上给它修改成30对吧？30。好，然后再检查一下left是16，然后其余吗这个是0left16，其余吗这个是0。磁力测量是30，没问题，ok再往下走了，他说将太子的最小和最大的宽度修改为320×49，这样子对吧？将它最大最小的宽宽度宽度是320320，然后我们的高度是4949，宽度320，高度这个位置它是4949给过来，320 49没问题，ok将please hold给它修改为选择视频描述，它的内部的文本修改为分类。好双击一下你那个是分类，然后将这个案子的普利斯的那么往下拉，好往下拉，维护的互动在这个位置对吧？他说将普利斯互动给它修改为选择视频描述，选择视频描述递过来，我们说给他看破boss将内部的那些像每个子项对吧？内容将来我们在程序当中给它手动来进行添加，因为现在我们的分类还没有对吧？好，这样子的话我们分类的这个位置也就把它处理完了，再往下来走到它底下，我们再来处理它的就是标签部分，他说拖拽一个q利普和q外地方到他可能探索当中左右摆。好好我们先拖拽提供一个拖进来完了之后再给它拖拽一个外力对吧？外力拖着进来，拖拽进来了之后可以把它稍微往小然后缩一下，拖拽进来了之后，然后接下来他要让我们将这套内部的从左往右修改为泰安格力布以及泰安格外力图，我就把它修改为泰安格力宝、JJ，label2太阳能力波，然后把它修改为太阳哥y里头，wid这一届，wid这一届。好，修改完了之后，我们再来看他说将标签的文本来修改为标签，然后给我也得从添加上一个背景颜色，ok你内部的文本把它修改为标签，顺便给我也给它添加一个背景颜色，我们就改成它的样式表bckgr都有肯定，当然格罗斯特杀了这个颜色，我们随便给它添加上一个颜色，比方说我把它改成这种绿色的分号栏加上确定ok它的颜色就上来了，再往后走了，他说选中它的疼痛，然后点击水平布局，对吧？将let修改为16，Dota的话修改为20，其余修改成0。好，那么先选中它的可能，点击水平布局完成了之后，我们接下来得需要修改它的插一个对吧？好向右槽往下拉往下拉。他说let修改的是16，包头修改的是20 20 20，其余全部让它修改成0，看一下 Let是16，包头是20，其余修改成04，VC银行修改成30，把 CC把它修改成30变成30。Ok完了之后再往下走了，他说将塔格利波的最小和最大的宽度修改成32，选中标签，然后往上拖，将最小和最大的宽度修改成32，最大的也把它给32对吧？32，好完了之后再让它往下来走。他说拖拽一个水平布局器，布局器到他在外地的当中，然后名字修改为他料子ok，拖拽一个水平不就行，因为他将来要在外地的当中去添加所有的标签了，对吧？所有的标签。好，我们把水平布局进来给它拖拽进来，点错了，然后把它往下拉，小一点好。放这进来了之后，将水平布局系统水平布局系统就不拆和内码把它修改为三个料子，还有a完不要紧对吧？三个料子。好，完成了之后的话，点击参观的时候，点击水平布局将吗？这个和CC给它全部修改成0，需要中谈个问题对吧？完了之后点击水平布局将它的优势马丁和CBC它全部修改成0，零你过来对吧？通过了那给完了之后我们再继续让他往下来走，注意看最后的步骤，然后我们拖出来一个不是把它到b天外地当中，将斋堂内把它修改为卡梅特，并且文本修改为提交。好，这个是我们到泰安格外里头，那么先给他拖着一个按钮进来，或这个按钮进来之后，把他的OPEC和内容修改为commit好妹特一天，然后将上面的文本给它修改为提交，文本修改成提交，文本修改成提交了之后，然后再拖拽两个水平弹簧放在冰箱的左右两侧，将来你需要依靠弹簧，就是把我们的按钮它挤到中间去对吧？挤到中间去，两侧点击水平布局，将Top吗修改为10包头，把它修改为34。Ok先选中，然后点击水平布局对吧？再选中选中往下拖，他说将left我看一下将Top修改为10，包头的修改为34，Top是10，包头修改成3434，我看一下包头修改成34，其余的吗？这张给他修改成00，然后这个也是0，这个有点不太对吧？我看一下。 Top设置为10刀头给他设置为34，然后后面还有将按钮的宽高给它最大和最小的宽高修改为136×48，ok把按钮给它选中，选中之后往上拖，最大最小的宽度修改成136 48，宽度136高度把它改成一个48对吧？48。你要再往下走了，还有我们的最大的宽度136，高度把它在这一块改成一个48就可以了，把它宰了给我48，这样子的话我们就将整个的宿舍上传视频页面的布局给它处理完了，对吧？它处理完了布局处理完了之后，下一步我们肯定要给当前的页面去设置样式对吧？设置样式在设置样式之前，我们先将每一个控件内部咱们之前所添加的这些背景颜色先给它清除掉，对吧？搞清楚掉。颜色我们到时候自己再给它重新来进行添加。好，我们的颜色基本上都是添加在 q利部或者说外力上的，我们一一个一次让他去点一下看一下，如果有背景颜色，我们就将这一些背景颜色给它移除掉就可以了，对吧？首先就是我们的已经我可以把它拿掉这里。好，然后这个是咱们给它简介打掉，对吧？ Qe宝看一下有没有没有，它也是一个管理。好拿掉。Ok再往下来走，冰贴管理好ok拿掉。然后还有我们都要看到我里头把它的背景颜色也清除掉，把它清除掉，再往下来走，我看一下就到达我们的分割线上也有它拿掉，好，太阳哥讨厌他，把他给拿掉，再往下来走就是我们的天安哥歪地图被颜色有拿掉，好往下拉。这个是抬头靠探讨背景颜色给它清除掉，然后还有开通boss也给他拿掉，确定再往下来走，就是我们的开始我看一下没有，他也投资大，他应该也没有，大家都contact box，ok拿掉对吧？拿掉再往后走了， blue的管理层咱们是最上方的上传，好，这个是o o的要给他拿掉。好，还有如果到外里头也给他拿掉。Ok，我看一下这个是把它往上推，我们刚刚清楚的是上午的位置，现在还剩了一个当艾肯 ok它把主持人拿掉对吧？拿掉之后我们上面这部分也把它煮的差不多了。再往下来走就是我们的video卡分背景颜色你看清除掉清除掉。好，再往下来走，还有康复boss。好，还有一个就是我看一下里面是利宝，看一下没有里面只os拿掉来再往下来走，它还有卡尔波boss，我看一下没有，还有卡巴利波把它删掉。好，再往下面走，我点一下，这个是我们给他买了一点。有对吧？有。还有现在就剩一个简介的位置，简介我看一下是博瑞不对，博瑞服务已经发6次，看一下66没有，这个应该是我们内部上给到，我看一下是不是他有把钥匙给他一清除他就清除，清除完了之后，从上往下我们顺便再给他去检查一下，把它往上抛，现在我们能够看到的叶脉当中的颜色已经全部被我们清除掉，对吧？颜色以已经被我们清除掉了。接下来我们给页面当中的每一个控件将样式能添加上来，这些样式我之前在文档当中已经整理好了，大家下来之后按照文档当中所给的样式逐个的把它添加上去就可以了，对吧？基本上我文档当中每一个空间的保护监测的内容和咱们刚才所添加的保护监测内容是一样的，大家找对人不健康内部所对的控件，然后把这些样式建议你自己最好还是手动的把它敲上去，不要让在这个地方拷贝对吧？每一个钥匙之前我们基本上都已经解释过了，差不多没有哪一刻哪一段就是没有解释过的。好，接下来就按照我们文章当中的样式，我们一个的让它往下来走去进行添加。来首先找到我们的主页面，我们先给它添加上一些通用的样式，因为这个页面当中碰见的比较多，而且有好些个文字，许多文字它的文字大小都是16个相搜点，我们统一把这些共性的部分把它提取出来，作为一个通用样式的跟进去。通用选择项就是星号对吧？星号完了之后 cmafamflf勒，这个方法能让他发给自己，自己就把它改成微软雅黑。然后就是方和赛斯对吧？字体大小了，我们就把它给16个小字完成了之后点击应用，这样子我们就将界面当中的这些文字给它统一了，当然有些文字还比较小的，后面再通过样式来进行调整。因为我们页面当中所有的控件都在QS，比如说are当中，接下来我们再给 QC安然就是把它的样式去设置一下， QS不是按照滚动条，我们在好些页面当中也对吧？也用到了，那么这个位置到样式上其实和我们之前在其他页面当中所设的样式上是一致的， SC ok1.2，首先它的边框我们不要壁画包装就把它设置为对吧？不同是领导，比方说这样的话，我现在走到我们去设置一下它的垂直滚动条q时刻让对吧？ Qs小孩不到，大的就是它的手柄，它右侧滚动条对吧？滚动条它有一个垂直滚动条，有一个水平滚动条，我们只去处理它垂直方向的 VR加cl，这个味道就是去处理垂直垂直滚动条对吧？垂直滚动条好。垂直滚动条它的边框我们也不要，也就是把她的包在设计完成了之后，然后当然不让的话，back这个od到c了等等，把是从井号f也就是改成白色，ok，然后把它的宽度wid接近尺宽度的话，我们把它改成10个像素，这样子的话我们垂直滚动条外观基本上就把它定型了，对吧了，再往下来走，我们还得需要注意一下，垂直滚动条块的手柄，然后这个是垂直滚动条，对吧？这可能是滚动条条不等，手臂上有什么托的或者掌注意什么？老师q l 4 PRO c不是l4l8对吧？它的手柄hndle然后是垂直方向上的vertfcl给完了之后不懂，高的我们也不需要，我就把它生成了，对吧？然后他来上个圆角的半径，bord包的 Ad rad us board readers，圆角半径把它改成5个像素点，再往下来走，再给它添加上1个就是背景颜色对吧？这个是b B包得说的是BC k这行都用，第八哥不让得到，c都完了到他了，背景压缩了，我们就把它改成井号，1818，对吧？这样子的话我们手柄就出来了，手柄出完了之后，然后让我们再去处理它，还是垂直方向的向上向与下剑谱把那个剑换给他，也和老师qscr都来了，q怎么说对吧？然后他的 up二AR W这个是vivertx点垂直方向的对吧？然后还有就是q四五克c我说4个肉棒基本上是down当larow中间它是不断往下划线，然后它是我们给的也是垂直方向的内地地，x类别就类似于上面这个点说底下这个点对吧？把你先去解决一下。Okay完成了之后，咱们把它的bod高的法也给他生成，还是等 Wid其实是把它的宽度在这个地方改成0的起速点，然后把它的高度对吧，高度也给它设置成0的小数点h1I h的g高度高度也设置成0的小数点，这样子的话，我们在里面就把它的垂直方向的滚动条处理了，对吧？滚动条的话就成这个样子，看到了吗？好，垂直方向的滚动条处理完了之后，水平方向什么也有，对吧？水平方向上只是咱们现在的元素没有把这个页面给它过分的重开，我们不管了，这个位置就是咱们把水平方向上的滚动条直接给它禁掉就可以了。这个位置是水平方向需要的时候，它就会出来，我们给它来上always of相当于始终把它关闭了，将来它就不会显示不显示这个是我们给它4个l它的滚动条得要12号再往下走，就到达了我们的地址 Pg。Ok，型号pgebg我们就把它的背景颜色就直接改成白色的，这个是backjroud background dot colr background dot color，里边是井号123456变成了6个x完了之后确定对吧？就是背景多少？背景有了之后再往下来走，我们从上往下来走，上来就到达文物道外立体。好，这个APP的外层它也是有自己的背景颜色的，对这个是好up LED录得aroo wic这一切对吧？录的yt，好，这个是井号对不对？是b1CK这一行都有，d班格乱的杠，co乱到他了，然后他这个颜色是警号，s 5s5s5对吧？F 5f5，然后运用也就是这种稍点深颜色的灰灰的。好，再往下来走，就是我们右侧它是有一个photo对吧？就是显示文件夹的对话框，在文件夹上显示在 photo上显示文件夹的图片了，这张图片它是在哪个位置？编辑找到我们的QS文件，我看一下qrc文件在哪一块，在这个位置把它们展开。在a max底下它有一个 Blue的，我们看一下 blue的video配置对吧？这样子一个在这个目录底下它有一个有一个文件你点给他一下对吧？到时候我们把设置进去，也就是说咱们将来在上传视频页面当中添加的所有图片都在，我们都没有配置到目录图像了，对吧？把这个模式关掉，关掉了之后，接下来我们将服务端给它选中。好，这个是井号 foldr跌到foldr这个是第一次见见都用，不然可乱动。有是包的，必有地包在里面去。然后 URL这个很好，刚才我们已经看过了，它是在我们的根目录底下也卖竹子的对吧？有一个 up Upload、微丢配置这个页面底下它有一个优势，文件点p对。好，应用这样子的话，这个图片它就上来了，对吧？图片上来了之后再往下面走了，还有我们的大陆的图片在这里，还有对吧？我们的当x当中井号down SC我们当艾特，它的内部也是一张图片，bordr包的fade那去，ok这个是URL同样也是在我们的根目录底下，这一块就是这个文件夹底下有一个路德利亚d楼没有配置对吧？一个配置，然后他在里面是完成完从两个平行分配完成两个平行线走过来，点一个应用，ok我们等完成了之后解决它上传完成了之后，你的文本要显示上传完成，对吧？右侧的一个图片它也要显示出来，它只有在视频上传完了之后才显示出来的。好，再往下来走，就是我们这一翻译好。井号这个位置就是文件的名字，它在里面是加粗的，就是方特WB谈这个事情，方特为他给出了biot，还有b然后再往下来走就是我们的方特杠赛斯对吧？字体的大小默认情况下16个相册，我们把它给了18个相册，打一下，好再往下走，就是我们文字的颜色擦了，他也就是前景色情况22，对。来点击一个应用，ok我们把这块也给它设置上来了，对吧？设置传播。好再往下来走，还有我们的就是上传进度这一块的文字，来把它先选中信号，它是阿布鲁德普鲁格suplad阿普罗德trodress对吧？如果s方特到SARS是16个小时，然后是我们给的文字的前景色c ol，他们这里面为警号二应用对吧？好。这样子的话我们在里边等于是视频显示的区域就把它给完了，给完了之后再往下来走，就到达我们的我的视频了，对吧？满意度我觉得这个是今后满video满v9 V9满v顶到它内部，当它到SARS SARS把它改成18个像素，然后完了之后字体的颜色它这里面是黑色的，那就是号井号0应用对吧？这个思想确实变大了一些，再往下来走，到达了我们的底下视频封面。好，这个视频封面我看一下是在 video卡特整个的 video卡通的区域，就是我们的视频封面内部以及它所对的一些文本，对吧？首先是我们只给了卡沃尼，我看一下卡伯尼伯在哪儿，利卡波利波我们又是给到这本文字对吧？我们的龙就是诗是对于图像的说明，系统默认选取失去了第一帧作为它的封面，那井号CEO美亚couple Mabe我们couple legal，好，帮他到赛场，或者刚才这个自强在里面是16个像素，主要就是它自己的颜色对吧？Col自己的颜色是9号9，然后点击应用稍微带点灰灰的这种颜色。Ok处理完了之后再来处理它的左侧，首先就是他给的星星，人家的星是红色的对吧？红色星号star也就是我们所给的4848方喝道SARS，把他们点成一个16秒的操作，然后是我们给到他了，他们在里面为井号ff006f ff006f点击一个应用，这个信号它的本质变化对吧？行为变化了，ok再往下来走，就是我们的他播太子警告 Ceo为银行差额暂时方特杠CS也是16个16，再往下走，他文字的颜色都是井浩二。好，这里对吧？当然我们在这个趋势没有看到它有多大的变化，我们想不到，好再往下来走就到达门的，你可以拿出力度，这可能是景色安倍之一，而AD一来了，这个点把地方我们把它设置成一张背景图像就可以了，必有必将包的杠IP包的a面吃，然后测的URL也过来了，那边也是在我们的根目录底下有一个a max的文件夹，在 a max底下，它一有一个就是upload录到am Bo video配置对吧？在这个页面底下它有一张就是分面部图像封面，两个图像应用，封面封面上没有看到，我们就到 A QS文件里面去看一下，对吧？我看一下。风莫依安庙演的天镇安佑我表态好，顺便我把他的名字复制一下，全部复制一下，复制完了之后把它关闭掉了之后，然后我们再来检查一下这个方式，a卖之李总，井号imhe然后这个是高的emerge URL，对吧？在我们的emerge底下有个up low的微观配置，ok。好，请坐，刚才应该是我们的头像名字写错了，可能写错了。好，再往下来走，到达我们按钮对吧？好，那么右键单击井号cham这一镇值1000，这个按钮它在里面是不要透明的，对吧？一瓦一二包的，我们先将它的边框在手上上，拿掉了之后Col把文字的颜色型号f文字的颜色我们把它改成白色的来应用对吧？对应用是称职必签。好，这样的话往下来走的话，我们再将它的因为这个按钮它也有默认的颜色，对吧？Back这一条都用d大家都知道的对吧？Cos它的我们把它的背景颜色也给它去设置一下，这个背景颜色是r GBA Aliah主要用的是它的冲动，离离000就是代表的是白色，黑色能把它改成0.00.0就代表这就是这个按钮，它的背景彻底是一个不同的点击应用，确定好像没有生效对吧？没有生效我们把这个按钮的咱们写错了，不是b签咱们的名字给他设button然后应用对吧？也就是我们现在看到的这种效果，ok这样子的话我们中间我的视频这一步知道内容也把它处理到差不多了对吧？差不多了再往下来走就到达我们的视频标题的位置，也就是开通访谈到boss，开通卡通的boss，拉进来了之后今后开口trttleconterg到登科 Bach's，我们先把它的边框的项目去设计，这种是b B包等对吧？报道这个波段的话线我就把它的线扳给成一个像素点，然后是solmid solid实心的颜色就把它改成井号，df和df e 1e6一个点这样的颜色来点击一个应用对吧？点击一个应用。Ok，这样它往下来走，我再来看一下这张画成这样，来点击一个应用，对吧？点子一个应用。Ok这样的话往下来走，我再来看一下这样好下来走到最后他对吧还有个信号，目前南极然后这个是我们井号给到开头，star txt tlestar对吧？开公司到好让他往下来走。当这段size把它们给成1个就是16个小镇对吧？16个小镇再往下走就是他的Cr了。文字的颜色也就是井号ff00，然后6s点过来点击应用对吧？点击应用它底下这个星号现在已经变成了红色，对吧？变成红色了。好再往下来走，我们给到 video test， video test了来注意看这是型号，这个是viveove6、geitgleve6test。他什么的方法，刚才自己在里面是16对吧？16。好，这样的话我这边走了，那就是文字的颜色，井号井号22对吧？ B也不 d波导我们把它的边框在这个地方拿掉就可以来选用，对吧？这里这样子的话我们就看到貌似给到剩余字数，总字数和已经添加的字数，它就是文本貌似它是讲的框框内容，能看来吗这个？从我们的样式当中它来进行的控制给外观，给外面外置方式，它把边框加上就把它对吧起来了。好，再往下来走，到达就是我们右侧这一段来通过等他也是有钥匙的，今后 left wrwrd left out what？ Left out他的方子，帮他杠SARS字体大小是16个享受，完了之后还有它字体的颜色，字体颜色字体的颜色是9号9，确定我们看一下 word， left word写错了就说怎么没有生效，word一般没有生效的话，要不然是你的前面的 ID写错了，要不然就是我们内部的样式的话可能出问题了对吧？样式出问题了，我的按照密码器我给他他点击一个应用对吧？这样子的话它的字你看一下这个字是不是就灰色的，好，再往下来走就到达了我们的简介的位置。那么先给他来上个先来看一下我们苦难猜测案例，我9号tlan不浪，爱怎么一定感谢你爱怎么？他进来之后biot o也包括我们重新设定一下一个销售点，然后是sorry的实心的对吧？颜色改成井号，dfdfe1e616把它推过来，完了之后再给它来上个圆角半径，dordr到radrus对吧？包到windows，包的瑞德斯，它在里面是4个像素2角的效果。好，再往下来走，还有他的，然后让他左侧文字内部的文字不要就是紧扣你的左边派拉丁let，我们比同时爱过销售，完了之后还有他的PDP安定派拉丁 Top了，我们也是会让他往下来的这种事情的想法，来点击并购对吧？它就离总和上就有了一定的距离了。再往下来走，还有我们底下给剩余对吧？剩余来这个是我们的不云服务来投过的警号，brief不是对吧？来和w我把d它的 Col Col8文字的压缩就是井号99 99 99，支持大小的默认就是16个像素大小是对16个对吧？现在这个不是灯带槽，他怎么也就变了，就会打扫。剪切部分完成之后再往下走了，应该就到达我们分类的位置了，ok把滚动条往下拖。托的时候，我们看到刚才选中的矿盘往下走往下走，我们先不管它应该是t里面的一个小话，好拨到左侧来，它主要就是一个com box的下拉框，我们主要也是给下拉框把它的样式设置好就可以了，首先将下拉框的卡选中了之后右键单击。不是这个下拉框我们在设置的时候主要是设置的是下拉框，它的一个外观，包括了它右侧的箭头，主要设置的是这两个。来大家看一下我们这样说该怎么给，首先就是我们的KFC探子，完了之后，dordr包德刚raradius我们将它的就是圆角半径给出来，对吧？4个小数点完了之后再让它往下走。这个边方边方我把它给成一个像素直线的story的对吧？颜色我们把它改成井号，dfe1、e6，我们上面的qnid盒以及 Plan就是多行选择的一个标便利泡沫，这样的两所是一样的，来再往下来走，就是它内部的文本，我们把它给它第一个 left它的net就是让文字离它的左边界有一些距离，对吧？我们点击一个应用就能看到了。我看这个是它在里面是d盘，从下方不应该是红的红的和我们的页面不太协调，ok把它给完了之后，接下来我们再将右侧下拉的部分对吧？已经把下拉的箭头给它做一下。好，下拉的箭头它属于康boss底下的一个就是子控件，对吧？子控件，comd从mbo Com books子控件的选择条，我们得需要用双引号了，两个冒号对吧？它是照谱杠杠下拉。好，咱们把它的宽度大概定金时宽度在这个的给出24个小数，高度hei这个系统也把它改成24个像素，也就是说它的宽高，将来我们的下拉的箭头它的宽高就成为正方形了，对吧？目前这样的话往下面走了b B包包的，然后我们把它拿掉，那就是设置了，好再往下来走，让我们给的键库的位置对吧？也就是三个子控件，子控件那就三个口处，co trol control子控件的位置，positio那么就让控件将来让它靠右，right？让你建筑靠右，你也可以让它靠左对吧？再往下来走要靠右侧的距离把它改成就是14个像素变成14个像素。好运作对吧？现在我们在里面看到了，已经把它的包装已经生成了，能看到键盘它在位置，没关系，我们给这个箭头在这个箭头上方贴上一张图片就可以了，对吧？这个东西qcomeo这个是comdo他们boss对吧？他底下的Dow当ler ow向下的箭头包得到MP之一到你那儿去，然后URL把图片的路径给过来，当然也是在我们的根目录底下有一个imagea那几个字对吧？叫文件夹。它底下有一个普路的阿普路德，v6上传视频的页面，你先把你到下拉点到pod的图片别管，点完了之后点几个应用对吧？确定这样子的话，我们的就是给到康boss它的一个编号，一二审就把它处理好了，对吧？之后然后再让它往下来走，就到达我们底下的标签部分对吧？分隔线还有我们到标签这个位置内部到样式上，我们现在没有办法去进行设计，因为它内部给的是一个个的按钮，这些按钮将来得需要我们根据分类的选择，选择好了分类之后，标签会才能将该分类底下的标签和动态的添加进来，我们再添加按钮的时候，顺便把那些按钮的一样添加上，对吧？添加上好，底下就像相对说两个，一个是我们的分隔线，一个就是我们底下的一个提交按钮。好，我先找一下我的分割线在哪，因为我们刚刚把分割线的颜色给它删除掉，删除了对吧？发现了现发只有一个像素，所以说不太好找不太好找，应该是三个位置对吧？三个问题ok，我们先把它折起来，在这个位置上找到了之后，然后右键单击改变20秒，这部分就是su separate Sepe Sepeig有 separate。我这里了。地滑地包的， Bc k都有安定巴哥格朗早擦了，我们只需要给它生成一个颜色就可以了。对吧？因为它现在只有一个小数点井号，第二组加dse1e616，我刚才和上次我们给了几个编辑框的边框，颜色给它一样的应用，这个线路它就出来了，对吧？它就出来了，这个线就出来了，ok最后还剩了一个我们，那么先把按钮给它选中了之后，然后这个是我给的com卡卡梅特BT对吧？考虑的BT。好这个按钮首先它得backgroud8杠cl它了，把 Background color给它设置成井号 cnecefecnecefe改完了之后才能它的CEO了，它文字的颜色，然后就把它改成白色的感受就好。这样的话我现在怎么再给它添加上一个原料干净，必有不一样对吧？包德刚对d盘us包括windows，包括改成4个小数点，点击应用对吧？我们的按钮就把它处理好，按钮处理好了之后，感觉底下这个就是非常窄的按钮，它都没有显示全对吧？好，看一下我们之前给的就是btny里头的位置，我们把它的最小宽度给的是4848，最大宽度也给它设置了，那么看一下能到是不是我们刚才的哪个东西它设置错了，按钮它是在最底下这个价格将拨出来一个btny里头， betny里头，它应该是在我们刚开始的位置设置的，对吧？我说我看一下最后是江滨天外地图的最小的高度设置的是什么？没有最大对吧？最大的我们就用它原先的ok这样子的话，我的整个的提交案例这个位置它看起来也比较协调了，对吧？还有一个问题，大家注意看这种它的风量好像也没有对齐是吧？也没有对齐，我看了一下开通考证态度，考虑开头考探头，他的life我们没有设置对吧？没有设置，然后上面标记的位置它的life我们给的是也没有设置不对，应该是上海这个位置对吧？中亚的位置。我看一下封面这个位置，它的有没有 Let，你给的是10对吧？ Let给等于10。那么底下开通卡特姆的net，也把它10改成10，看一下我才能给出来。10这样的话它就朝里面缩了一下，缩进来了之后，我们看到它这一层就是上下在里面对齐了，对吧？好，这样子的话，我们就将整个的上传视频页面它就处理完了，复制完了之后，接下来我们现在在QQ当中利用它给我们提供的预览器，就是把这个窗口给它去预览一下，对吧？假设我要看一下它的温度风光，ok那么窗口将来它长的就是这种样子对吧？把它往下滑，往下滑，将来要么在动态的动态的将其内部的标签分类添加进去，添加进去了之后，只要用户一选择在标签的位置，就会将当前分类底下的标签给他，对吧？显示出来了，上传视频页面我们就把它处理好。”