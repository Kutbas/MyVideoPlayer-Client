...
在前文中，我们已经讨论了 Qt 自带的 QMediaPlayer 在简单视频播放场景下表现良好，但在处理流媒体、高清视频、复杂播放控制等场景时存在明显局限。因此，本项目最终选择 **mpv** 及其嵌入式库 **libmpv** 来构建高性能的视频播放组件。

本文将从 mpv 本体开始介绍，包括它的特点、功能、配置方式，并结合课堂演示内容说明如何使用 mpv 播放文件、播放流媒体，以及如何在开发中使用 libmpv 嵌入播放器。

**1. mpv 与 libmpv 简介**

**mpv** 是一个免费、开源、跨平台的命令行媒体播放器，由 MPlayer 和 mplayer2 项目衍生而来。它不仅自带强大的播放能力，还具有高度可定制化、脚本化、硬件加速完备的特点。

**libmpv** 则是 mpv 官方提供的嵌入式库，使开发者可以将 mpv 的成熟功能直接集成到桌面应用、移动应用或嵌入式系统中。

**2. mpv 的核心特点**

**(1) 界面简洁，聚焦播放内容**

mpv 默认没有传统播放器的 UI，而是通过最小化界面让用户专注于内容本身。
 同时它支持主题（skin）与外观自定义，可通过配置文件（`mpv.conf`）实现 UI 风格扩展。

**(2) 高性能、低开销**

mpv 的性能表现非常突出：

- 支持多种硬件解码（VAAPI、VDPAU、NVDEC、VideoToolbox 等）
- 能充分利用 GPU 渲染管线
- 播放流畅性高、资源消耗低，适合长时间播放或低功耗设备

这一点尤其适合我们的项目场景（流媒体播放 + 多系统兼容）。
...
- 几乎所有音视频格式（MP4、MKV、AVI、RMVB…）
- 多种视频编解码器
- 字幕加载与渲染
- 精准倍速控制、AB 循环、截图、滤镜等

它在功能丰富程度上远超 Qt Multimedia 模块。

**(5) 强大的脚本系统**

mpv 内置 Lua 脚本引擎，可通过脚本实现：
...
因此 mpv 更像是一个“播放器框架”，而不仅仅是播放器。

**(7) 对开发者友好：libmpv**

mpv 提供的 **libmpv** C API 使其可以直接嵌入到应用程序中。

libmpv 的优势包括：

- 可在 Qt、GTK、Win32、Android、Python 等环境中集成
- 使用简洁的消息循环与属性系统控制播放
- 底层解码由 FFmpeg 完成，功能完备而稳定
- 可轻松构建专业级播放器（如 Bilibili、YouTube 客户端等）

mpv GitHub 仓库也提供了丰富的示例，适合作为学习和开发参考。

**3. 官方资源**

- **mpv 官网**：mpv.io
- **mpv GitHub 源码**：github.com/mpv-player
- **libmpv 文档**：官方提供 C API 示例与集成指南

你可以从 mpv 官网下载对应系统的播放器，并直接运行测试功能。

**4. mpv 的实际使用演示**

课堂上老师进行了详细演示，这里进行整理与技术化描述。
...
mpv 即可立即播放本地视频文件，支持快速拖拽操作。

**(2) 播放流媒体（M3U8 分片）**

例如课堂中之前用 Qt QMediaPlayer 无法正确播放的：
...
```bash
mpv --geometry=1024x860 ./111.mp4
```

更多窗口控制参数可以通过配置文件默认设置。
...
mpv 的快捷键全部可自定义，并可通过脚本扩展更多功能。

#### Qt中使用mpv库进行视频播放

在多媒体应用开发中，视频播放功能是常见的需求。Qt框架虽然提供了自己的多媒体模块，但有时我们需要更强大的播放能力，这时可以考虑使用mpv库。mpv是一个基于MPlayer和mplayer2的开源媒体播放器，提供了丰富的API供开发者集成到自己的应用中。

**1. 环境配置与基础设置**

在开始使用mpv之前，我们需要进行一些必要的环境配置。首先要注意的是本地化设置，这是一个容易被忽略但十分关键的步骤：
...
为什么要设置`LC_NUMERIC`区域呢？这是因为不同地区对数字格式的处理方式不同。比如在英语环境中，数字"1234.56"使用点号作为小数点，而在德语环境中则使用逗号。mpv库内部需要统一的数字格式解析，将其设置为"C"标准可以确保使用ASCII编码和格式化规则，避免因数字格式不一致导致的解析错误。

**2. 创建mpv实例与界面布局**

创建一个基本的视频播放器界面需要完成两个主要任务：构建用户界面和初始化mpv实例。

在构造函数中，我们首先设置窗口的基本属性：
...
要让mpv知道在哪里渲染视频，我们需要创建一个专门的容器窗口，并获取其原生窗口句柄：
...
这里有几个关键点需要注意：
- `Qt::WA_NativeWindow`属性确保Qt会为窗口创建操作系统原生的窗口句柄
- 不同操作系统下窗口句柄的处理方式可能不同，Windows系统需要确保句柄不会为负数
- 将窗口ID传递给mpv的`wid`选项后，mpv就会将视频渲染到这个指定窗口

**(3) 配置输入选项**

mpv提供了丰富的输入控制选项，我们可以根据需要启用或禁用：
...
- `input-default-bindings`启用mpv自带的快捷键绑定，如空格键暂停/播放、方向键调节音量
- `input-vo-keyboard`启用视频输出窗口的键盘输入功能，允许用户直接在视频画面上使用键盘控制

**(4) 属性订阅机制**

mpv的属性订阅机制类似于关注公众号 - 只有订阅了特定属性，当这些属性发生变化时我们才会收到通知：
...
这里订阅了三个重要属性：
- `time-pos`：播放时间位置，用于显示当前播放进度
- `track-list`：音视频和字幕轨道列表，用于轨道切换功能
- `chapter-list`：章节信息列表，用于章节导航

**(5) 日志系统配置**

为了方便调试和状态监控，我们可以配置mpv的日志系统：

```cpp
    mpv_request_log_messages(mpv, "info");
```

这将让mpv在生成信息性日志消息时通过事件机制通知应用程序，我们可以将这些日志显示在之前创建的日志窗口中。

**4. 事件处理机制**

**(1) 回调函数与信号槽连接**

mpv使用回调机制来通知应用程序有事件需要处理。我们首先定义一个唤醒函数：
...
这个函数会被任意的mpv线程调用，目的是通知Qt的GUI线程唤醒事件处理。由于跨线程操作，我们使用Qt的信号槽机制来安全地传递事件：
...
这里使用`mpv_wait_event(mpv, 0)`，第二个参数0表示立即返回，不会在没有事件时阻塞。

**(3) 具体事件处理**

不同类型的事件需要不同的处理方式。我们通过switch语句来区分处理：
...
通过文件对话框选择要播放的视频文件，然后使用mpv的异步命令加载文件：
...
`mpv_command_async`函数会立即返回，不会等待命令执行完成，命令执行结果通过事件机制返回。

**(2) 创建新窗口**

为了方便多视频播放，我们提供了创建新窗口的功能：
...
- 确保mpv的动态库在程序运行时可用，通常需要将dll文件放置在可执行文件同级目录
- 不同构建系统（qmake、CMake等）配置mpv依赖的方式有所不同
- 跨平台开发时需要处理不同操作系统的窗口句柄差异
- 合理处理mpv事件的线程安全问题，确保GUI操作在主线程执行
...
因此，理解各 API 的使用方式和调用顺序，是在项目中成功集成 MPV 的关键。

下面整理本项目中用到的主要 MPV C API 方法，并结合课堂讲解对其使用背景和注意事项进行说明。

**1. mpv_create：创建 MPV 实例**
...
创建一个 MPV 的“实例句柄”，可理解为打开了一个和 MPV 播放器交互的后台对象。后续所有操作都基于这一句柄完成。
...
1. **内存不足**
2. **程序的 LC_NUMERIC 未设置为 "C"**

特别是第二点常被忽略：
 MPV 内部依赖小数点使用英文格式（`.`），因此在构造实例前必须确保：

```c
setlocale(LC_NUMERIC, "C");
```

若未设置，可能导致解析数值失败，进而导致 `mpv_create` 创建失败。

**2. mpv_terminate_destroy：销毁 MPV 实例**
...
用于 **终止播放器并销毁实例**，执行过程包括：

1. 向 MPV 播放器发送退出命令
2. 等待播放器完全停止
3. 释放 `mpv_create()` 创建的实例内存
...
必须在视频播放结束、应用退出或播放器不再需要时调用，以防止资源泄露。

**3. mpv_set_option：设置启动选项**
...
| ------ | ---------------------------------------------------- |
| ctx    | mpv 实例                                             |
| name   | 参数名称，例如 `"input-default-bindings"`            |
| format | 值类型（如 `MPV_FORMAT_INT64`, `MPV_FORMAT_STRING`） |
| data   | 指向选项值的指针                                     |
...
```c
int flag = 0;
mpv_set_option(ctx, "input-default-bindings", MPV_FORMAT_FLAG, &flag);
```
...
```c
int enable = 1;
mpv_set_option(ctx, "input-vo-keyboard", MPV_FORMAT_FLAG, &enable);
```

**(4) 注意**

**必须在 mpv_initialize() 之前调用**。
初始化后设置将不再生效。

**4. mpv_observe_property：订阅属性变化事件**
...
用于监听播放器某个属性的变化。
 当属性值改变时，MPV 会向事件队列发送：

```
MPV_EVENT_PROPERTY_CHANGE
```

并递送对应的 `mpv_event_property` 数据结构。
...
- `reply_userdata`：自定义 ID，事件回调时会带回，可用于区分不同属性
- `format`：属性值的类型，例如 `MPV_FORMAT_DOUBLE`
...
在事件回调中，你可以根据 `format` 判断数据类型，再从 `data` 中取值。

**5. mpv_set_wakeup_callback：注册事件回调函数**
...
MPV 在内部状态发生变化或有事件需要处理时，会调用这个回调函数。
 回调通常不在主线程触发，因此必须“唤醒”主线程来处理事件。
...
完成 MPV 的内部初始化，使其进入可播放状态。

**(2) 注意事项**

- 必须在初始化前完成所有 `mpv_set_option` 设置
- 初始化后才能发送播放命令（如 `loadfile`）
- 初始化后可以开始订阅属性、发送事件
...
2. `mpv_create()`
3. `mpv_set_option()` 设置启动参数
4. `mpv_observe_property()` 订阅属性
5. `mpv_set_wakeup_callback()` 注册事件回调
6. `mpv_initialize()`
7. 使用 `mpv_command_async()` 播放文件
8. 使用 `mpv_set_property_async()` 修改属性（如暂停、跳转、音量）
9. 事件回调中处理 `mpv_wait_event()` 返回的事件
10. `mpv_terminate_destroy()` 退出与清理

**9. 关于命令执行：mpv_command_async**

尽管你提供的内容中未给出函数声明，但课堂明确讲了：

- `mpv_command_async` 是异步命令接口
- 用于执行 `loadfile`、`seek`、`stop` 等命令
- 命令执行结果由事件系统异步返回
...
本节课程主要围绕 MPV 播放器的 **音量属性配置**、**界面交互设计** 以及 **信号槽联动逻辑** 展开，实现完整的音量调节功能，包括点击、拖拽滑杆时实时更新音量大小。

MPV 本身提供了一个 `volume` 属性，用来控制输出音量，其取值范围为 **0~100**。我们只需要通过 `mpv_set_property_async` 将计算好的音量值传入即可完成音量调整。

**1. MPV 播放端的音量属性支持**

为了让播放器具备调节音量的基本能力，需要在 `MpvPlayer` 类中提供一个设置音量的方法。该方法本质是对 MPV 的 `volume` 属性赋值，如板书所示：
...
课堂上特别强调过一点：**MPV 的 volume 属性使用 `int64_t` 类型**，因此我们在 Qt 调用时也必须使用一致的数据类型，否则会出现参数传递截断，导致音量看似变化但实际无效的情况。这个问题在调试阶段也确实出现过，当数据类型从 32 位改为 64 位之后，音量控制才成功生效。

**2. PlayPage 中对音量信号的响应**

播放器界面 `PlayPage` 需要接收来自音量调节窗口的信号，并将最终的音量值传递给 MPV，使用 Qt 的信号槽机制非常自然：

```cpp
connect(volume, &Volume::setVolume, this, &PlayPage::setVolume);
```

`setVolume` 槽函数将音量值继续传给 `MpvPlayer`：

```cpp
void PlayPage::setVolume(int volumeRatio)
{
    mpvPlayer->setVolume(volumeRatio);
}
```

至此，界面已经可以根据音量调节窗口的输出，完成对播放器音量的设置。

**3. 在 Volume 音量窗口中处理滑杆操作**

音量窗口是本功能的核心逻辑层之一，它不仅负责展示 UI，还需要在鼠标点击、拖动滑杆时实时计算音量，并通过信号告知主界面更新音量。

音量窗口初始化时会设置一个默认音量值：

```cpp
Volume::Volume(QWidget *parent)
  : QWidget(parent), ui(new Ui::Volume), volumeRatio(35)
{
    // ...
}
```

然后通过事件过滤器对滑杆区域的鼠标事件进行处理：
...
课堂讲解中重点指出：音量窗口内部必须实时触发音量信号。

- 当鼠标点击滑杆时，需要先计算音量大小；
- 当拖动滑杆时，每次移动都要重新计算音量并发送信号；
- 当鼠标释放时，也需要发送一次信号，确保最终音量值落实到主界面。

因此，`eventFilter` 在三个事件分支中分别完成了不同级别的音量更新触发逻辑。

**4. 计算音量值：由滑杆位置转换为 0~100 的音量**

音量窗口通过 `calcVolume()` 方法实现“滑杆位置 → 音量值”的换算：
...
1. UI 中的滑杆高度大约是 **145 像素**，但音量值的范围是 **0 ~ 100**；
2. 因此不能直接把像素位置作为音量，需要做归一化；
3. 使用双精度计算可以避免整数除法导致的精度丢失；
4. 滑杆是从上到下但音量是从大到小，所以需要用 `100 - (...)` 做反向换算。

**5. 完整流程总结**

将上述内容串联之后，音量调节的完整路径是：

1. 用户在 `Volume` 窗口中点击或拖动滑杆。
2. Volume 通过 `eventFilter` 捕捉事件，调用 `calcVolume()` 计算音量大小。
3. Volume 使用 `emit setVolume(volumeRatio)` 发射信号。
4. PlayPage 捕获 `setVolume` 信号，调用自己的槽函数更新音量。
5. PlayPage 将音量值传递给 `MpvPlayer::setVolume`。
6. MPV 最终通过 `mpv_set_property_async` 完成声音大小调整。

经过调试和修正数据类型不一致的问题后，这一功能已经能够正常工作：滑动音量滑杆时，声音能实时随之变化，控制体验顺畅。

#### 播放进度实时同步

在播放器完成音量调节能力之后，另一个不可或缺的功能就是——让播放进度条与当前播放时间能够**实时**更新。随着视频的播放推进，用户理应看到底部的进度条不断改变，同时当前播放的时间也应同步刷新。若缺少这一机制，播放器将无法准确反映视频播放状态。

实现播放时间同步的关键在于：**提前订阅 MPV 的 `time-pos` 属性**。这个属性代表“当前播放时间（秒）”，MPV 在视频播放中会不断更新它。只要我们订阅了这个属性，当 `time-pos` 发生变化时，MPV 会触发一个 `MPV_EVENT_PROPERTY_CHANGE` 事件。我们捕获这个事件，从中提取 `time-pos` 对应的新值，再通过信号通知界面刷新播放时间与进度条。

**1. 在 MpvPlayer 中订阅 time-pos**

板书中展示了订阅方式。一般我们会在 MPV 初始化之后、开始播放之前完成订阅操作：

```cpp
mpv_observe_property(mpv, 0, "time-pos", MPV_FORMAT_INT64);
```

课堂上解释过，MPV 拥有多个常用属性，如视频总时长 `duration`、当前播放位置 `time-pos`、音量 `volume`、静音状态 `mute`、播放速度 `speed`、暂停状态 `pause` 等。而 `time-pos` 就是进度更新中最核心的属性。只要注册完成，MPV 就会在播放过程中自动监控它，一旦时间发生变化便触发事件。

**2. 处理 MPV 的事件并获取播放时间**

订阅属性之后，MPV 会不断向我们发送事件。我们在 `handleMpvEvent` 中对事件进行处理，当事件类型是 `MPV_EVENT_PROPERTY_CHANGE` 时，即表示某个已订阅属性发生改变。
...
课堂上花了不少时间讲解 MPV 事件结构体。事件本身使用 `mpv_event` 表示，其中 `event_id` 用来标记事件类型，比如属性发生变化时就是 `MPV_EVENT_PROPERTY_CHANGE`。事件内部的 `data` 可能是不同结构体，而对于属性变化事件，它实际是一个 `mpv_event_property`。

`mpv_event_property` 中包含三个核心字段：

- `name`：发生变化的属性名称，如 `"time-pos"`
- `format`：属性的数据格式，如 `MPV_FORMAT_INT64`
- `data`：属性的值。若是 `time-pos`，其内部就是一个 `int64_t` 指针，表示当前播放时间（单位为秒）

只要判断名称是 `"time-pos"`，就能从 `data` 中取出播放时间，并通过信号发给界面。

**3. 将播放时间同步给 PlayPage**

在播放器界面 `PlayPage` 中，需要接收 MpvPlayer 发出的 `playPositionChanged` 信号。板书中的代码展示了绑定方式：

```cpp
connect(mpvPlayer, &MpvPlayer::playPositionChanged, this, &PlayPage::onPlayPositionChanged);
```
...
目前只是打印播放时间。实际使用中，这个时间会用于更新界面显示，例如将秒数格式化成 `mm:ss` 或 `hh:mm:ss`，再让进度条根据时间比例同步移动。

课堂讲解里强调过：界面层的更新非常直接，只要拿到时间，就能更新标签控件和进度条。真正复杂的部分反而是“如何从 MPV 获取播放时间”，而这部分正是订阅属性与事件机制的作用。

**4. 播放进度条由 PlaySlider 单独处理**

在界面显示中，播放时间负责展示“当前是第几分钟第几秒”，而进度条负责展示播放进度比例。课堂上强调，实际移动进度条的是 `PlaySlider` 类，而不是 `PlayPage` 自身。

当 `PlayPage::onPlayPositionChanged` 被触发后，PlayPage 会将时间传递给 PlaySlider，由它完成位移计算与更新。这样做是为了模块化设计，让进度条控件独立处理自己的视觉逻辑，而不是把所有代码堆在 PlayPage 中。

**5. 播放时间格式化与界面显示更新机制**

在实现播放时间的实时同步之后，播放器还需要将 MPV 返回的秒数转换为更直观的时间格式，例如 `00:05` 或 `01:02:33`。这不仅使播放界面更加易读，同时也是进度条和时间标签更新逻辑的重要组成部分。由于 MPV 给出的时间单位是秒，我们必须自行将其转化为“时/分/秒”结构的字符串。

在播放界面的实现中，PlayPage 负责时间格式化与界面更新。板书中的代码展示了这个功能的核心方法，即 `secondToTime`，它将秒转化为格式化后的时间字符串：
...
课堂上老师强调过，MPV 的 `time-pos` 返回的时间是以秒为单位的整型，因此只需要对这个数字进行除以 60 和取模就能得到分钟和秒，而小时部分只有在视频时长超过一小时后才会显示。格式化使用的是 `QString::asprintf`，这与 C 语言中的 `printf` 十分类似，通过 `%02lld` 的方式保证个位数能够补零。

完成格式化之后，播放页面就能够准确展示“当前播放时间 / 视频总时长”。在测试阶段，老师将总时长写死为 10 秒，代码如下：

```c++
void PlayPage::onPlayPositionChanged(int64_t playTime)
{
  QString curPlayTime = secondToTime(playTime);
  QString totalTime = secondToTime(10); // 测试阶段写死的总时长

  ui->videoDuration->setText(curPlayTime + "/" + totalTime);
}
```

当进度同步机制运行后，这段代码会在播放过程中持续更新界面底部的时间标签。课堂上在调试时能够看到输出窗口中不断打印的时间，这也说明 MPV 的 `time-pos` 属性已经正常工作。

不过在实现过程中也遇到了一些典型问题。老师在课上调试时，播放页面初次加载但视频尚未开始播放，此时 `time-pos` 的值实际上是不存在的。MPV 在这种情况下会触发属性变化事件，但其中的 `data` 字段是空指针。如果不加以判断，程序在读取数据时就会直接崩溃。课堂上就因为这个问题导致程序启动后立即崩溃。解决方法是对空数据进行过滤：
...
这一句看似简单，但缺少它就会造成非常隐蔽的内存错误。老师强调这是 MPV 事件处理中最容易忽视的陷阱，只有当播放真正开始时 `time-pos` 才有意义，否则任何访问都属于非法行为。

调试时还出现过因为 `switch-case` 中缺少 `break` 导致事件掉入后续分支的情况。这类错误会使程序在处理属性变化事件后继续执行下一个分支甚至越界，从而造成 MPV 对象被误操作或被销毁。程序崩溃后老师使用断点调试方式逐步检查事件中结构体的字段，在展开 `mpv_event_property` 后可以明显看到 `data` 为空，从而确认问题原因。加入过滤条件后程序就能顺利启动并进入播放页面。

当视频开始播放后，终端与界面都能看到秒数不断更新。示例视频总时长为 10 秒，因此界面上会显示：
...
这一阶段的结果表明播放时间同步和格式化功能都已经完全正常，下一步就可以将格式化后的时间与进度条关联起来。进度条的处理由 PlaySlider 独立负责，而 PlayPage 的职责主要是管理时间显示。这样的模块化设计使得代码结构更加清晰：MPV 负责触发事件、PlayPage 转换与展示时间、PlaySlider 负责视觉进度同步。

至此，播放器已经具备了实时更新播放时间的能力，并且能够优雅地将秒数格式化为标准的时分秒显示，为后续实现拖动进度条、跳转播放位置等功能奠定了基础。

**6. 将播放时间映射为进度条宽度**

播放时间已经能够从 MPV 那里拿到，下一步就是把时间映射为可视化的进度条。核心思想非常直接：用当前播放时间与总时长的比率乘以进度条可用的最大宽度，从而得到进度条当前应显示的像素宽度。进度条的更新逻辑被封装在 `PlaySlider` 中，板书里的方法就是这个逻辑的实现核心：
...
这段代码做了三件事：把比率乘以 `inLine`（进度条背景或容器）的当前宽度得到 `playProgress`，记录或打印计算结果，然后调用 `moveSlider()` 去把滑块/已播放条视觉上移动到 `playProgress` 对应的位置。这里 `stepRatio` 是一个 0~1 的双精度比率值（例如播放到总时长一半时是 0.5），`ui->inLine->width()` 返回进度条可用的像素宽度。

课堂上对这段逻辑的讲解与实践集中在几个关键点上，这里把它们整理成易读的说明并补充常见问题的排查方法。

首先，**比率的计算**应该在 PlayPage 层完成（PlayPage 已经能接收来自 `MpvPlayer::playPositionChanged` 的秒数）：
 将 `playTime`（当前秒数）除以 `totalTime`（视频总秒数）得到 `stepRatio`，然后把 `stepRatio` 传给 `PlaySlider::setPlayStep`。务必使用浮点运算（`double` 或 `qreal`）以避免整数除法造成的精度丢失或直接为 0 的问题，例如：

```
double ratio = (double)playTime / (double)totalTime;
playSlider->setPlayStep(ratio);
```

课堂调试中最常见的问题就是进度始终不动，或者一开始就是 0。老师指出可能的原因主要有两类：

1. **ratio 为 0 或 totalTime 为 0**
   - 如果 totalTime 在测试阶段写死或未正确读取为实际视频时长，除法会导致 0 或除以 0 的不合理值。需要确保 totalTime 在调用前已经被正确设置并且非零。
   - 在计算 ratio 时强制转换为 `double`，并在传入 `setPlayStep` 前做范围约束（0 到 1），以防异常值。虽然板书不修改代码，但在 PlayPage 调用处加入检查能避免问题。
2. **ui->inLine->width() 为 0（或未按预期初始化）**
   - 如果进度条容器的宽度为 0，那么 `playProgress = stepRatio * 0` 始终为 0。课堂上提到在 UI 还没布局好或刚创建时可能出现这种情况。解决思路是：确保在窗口显示/布局完成后再开始基于宽度的计算，或者在 `setPlayStep` 中用一个合理的“最大宽度”回退值（例如布局完成前使用默认宽度），并在窗口 resize 或 showEvent 时重新计算并更新一次进度。
   - 实际项目中常见做法是：在 widget 的 `resizeEvent` 或 `showEvent` 中把宽度缓存到变量里，或者在 `PlayPage` 中在主窗口显示后第一次收到 `playPositionChanged` 再调用 `setPlayStep`。课堂上也演示了用打印（LOG）逐步定位出问题：如果 `playProgress` 一直打印为 0，那么问题很可能出在 `inLine->width()` 或 `stepRatio` 上。

老师在课上还演示了如何用日志追踪来定位调用/参数问题：先在 `setPlayStep` 打印 `stepRatio` 和 `ui->inLine->width()`，再打印 `playProgress`，这样可以明确是比率不对，还是宽度为 0，或者 `moveSlider()` 没有正确响应。三处打印能把问题快速缩小到具体环节。

另外，要注意视觉初始化：进度条默认应该显示为 0（未播放时宽度为 0），随着播放增长逐步增加；但若你在 UI 设计时不小心把“已播放条”默认宽度设为了某个固定值（比如 450），则乘以比率后会出现畸变。课堂建议把进度条的初始宽度设为 0，并在样式或布局中让容器宽度随窗口调整而变化（这样 `ui->inLine->width()` 始终代表当前可用宽度）。

最后，关于 `moveSlider()`：这一步是把 `playProgress` 应用到控件上（移动滑块、调整已播放条的宽度等）。如果 `playProgress` 正确但界面不动，要检查 `moveSlider()` 是否真正使用了 `playProgress`，以及是否在 UI 线程中执行（Qt 的 UI 更新必须在主线程）。课堂上提醒，若 `moveSlider()` 涉及动画或样式属性改变，也要确保调用后触发重绘（例如调用 `update()` 或 `repaint()`），不过这些实现细节由 `PlaySlider` 内部维护，外部只需保证传入的数据准确。

**7. 支持视频播放结束后的“重新播放”功能**

在本节课中，我们讨论了播放器界面在视频播放完成之后应当呈现的逻辑状态。一般来说，视频播放到末尾后，播放时间与进度条都会同步更新，但此时播放器的状态并没有自动回到可重新播放的起点，而是停留在“播放结束但未重置”的尴尬状态：用户点击播放按钮也不会使视频重新播放。这显然不符合常见的用户体验设计。

因此，本节课的核心目标，是让播放器在视频播放结束之后：

1. **自动将播放按钮图标改为“暂停状态”**（即视觉上显示停止）
2. **内部状态标记为暂停（isPlay = false）**
3. **在用户再次点击播放时，可以重新从头播放视频**

而这一功能的实现关键，正是围绕播放时间（playTime）进行判断与处理。

**(1) 播放结束时的逻辑处理**

在板书代码中，播放位置的实时更新通过 `onPlayPositionChanged` 方法完成。视频每播放一帧，都会触发该回调。因此，检测“播放是否结束”最自然的方式，就是在这里判断 `playTime` 是否等于总时长。

在课堂示例中，总时长被简化为 10，当播放时间到达 10 时，就认为视频已经播放完毕。于是播放完成后的处理如下：
...
- **把 isPlay 设置为 false**，表示当前已不是播放状态；
- **将播放按钮图标替换为暂停图标**，使界面状态与实际逻辑一致。

这样，用户在视频放完后看到的播放按钮，会自动回到“暂停状态”。

**(2) 让视频能够“再次播放”**

仅仅修改图标和状态还不够。用户期待的是：视频结束后，如果他们再次点击播放按钮，视频理应从头开始播放。

重新播放的关键判断写在 `onPlayBtnClicked` 里：

```c++
if (playTime == 10 && isPlay)
{
    // 播放位置修改到起始，用户点击播放按钮可以重新播放
    this->playTime = 0;
...
1. **将播放时间清零** —— 播放应该从头开始。
2. **重新调用 startPlaying(videoPath)** —— 加载一次完整的视频文件。
3. **调用 mpvPlayer->play()** —— 实际触发播放动作。

课堂上也特别强调了：
 `startPlaying` 方法内部会重新加载视频，因此重新播放的场景下，只需要再次调用它即可。为了能够成功加载路径，视频路径必须在首次播放时保存下来：
...
正因为保存了 videoPath，播放器才能在“重新播放”时重新加载视频。

**(3) 播放状态的衔接：为什么重新播放时可能“不动”？**

课堂实际运行程序时，曾遇到一个典型问题：
 视频重加载之后虽然已经进入播放状态，但画面并没有动。后来我们发现，这主要是因为：

- **播放结束时 isPlay 被设为 false**，代表“暂停状态”；
- 但重新加载后，播放器内部依然保持“暂停”状态，因此不会自动播放；
- 必须在重新加载后再调用 `mpvPlayer->play()` 才能真正播放。

因此在重播逻辑中，“加载”与“播放”必须明确区分并完整执行：

1. 先让 startPlaying() 重新加载视频
2. 再调用 mpvPlayer->play() 发起播放动作

否则就会出现“已经加载但没有播放”的现象。

#### Seek 功能实现

在前面我们已经实现了播放时间的同步更新，而本节课的主要任务是让播放器真正支持 **seek 功能**：也就是说，在视频播放过程中，用户可以通过点击或拖拽进度条，自由设置播放位置。这个功能看似简单，但其背后牵涉到界面层的交互处理以及对 mpv 播放引擎的属性修改，两者需要配合才能使 seek 行为顺畅工作。

**1. Seek 功能的整体思路**

“Seek” 的本质是 **根据用户在界面上的操作位置，计算出对应的播放进度比例，再将此比例换算成视频时间并设置给播放器**。老师在课堂中反复强调：实现过程始终是“两步走”：

1. **界面层负责获取比率**：用户在进度条上点击或拖拽松开时，我们要取得 `当前宽度 / 总宽度` 的比例（如 0.35）。
2. **播放器层负责设置时间**：拿到比率后，将其 *乘上视频总时长* 得到时间值，再调用 `mpv_set_property_async` 设置到 `"time-pos"` 属性。

也就是说，界面决定 “你想跳到哪里”，mpv 决定 “跳到哪个时间点”。

因此，我们先实现 mpv 层，再处理界面层。

**2. mpv 层：设置当前播放位置**

老师在讲解时说明，mpv 中控制播放位置的属性就是 `time-pos`，单位是秒。因此在 `MpvPlayer` 类中，我们需要添加一个设置播放位置的方法，其作用是：

- 参数：秒数（`int64_t`）
- 设置 mpv 的 `"time-pos"` 属性
- 使用 `MPV_FORMAT_INT64`
...
这部分负责“你告诉我要跳到第多少秒，我就跳过去”。

**3. 界面层：从进度条获得播放比例**

界面部分主要由 `PlaySlider` 和 `PlayPage` 两个类协同完成。

**(1) 鼠标释放时计算比率并发出信号**

无论用户是点击还是拖拽，只要松开鼠标，我们就可以认为他们已经确定好进度位置。这一刻我们获取当前进度条宽度，通过：
...
老师在课上也提到，如果之前变量名有不规范之处应进行修正，这部分我们在原代码中已经保持了正确写法。

**(2) 信号连接到 PlayPage**

进度条发出信号以后，由 `PlayPage` 来真正处理该进度值。因此在构造函数中，我们绑定：

```cpp
connect(ui->videoSlider, &PlaySlider::setPlayProgress, this, &PlayPage::setPlayProgress);
```

**4. PlayPage：由比率计算播放时间并调用 mpv**

在 `PlayPage::setPlayProgress` 中，我们将比率转换成具体秒数。例如演示视频时长为 10 秒，就直接：
...
除了 seek 功能，本节课还新增了一个常见快捷操作：按空格切换播放/暂停。实现方式是通过 `QShortcut` 捕捉空格键，并让播放按钮执行一次模拟点击。
...
课堂上老师特别提到 lambda 必须用 `[=]` 或 `[&]` 捕获 this，否则编译会报错 —— 板书中采用了 `[&]`，已解决该问题。

#### 弹幕功能设计与实现

在前期的开发中，我们已经实现了播放页面的一系列基础功能，如播放/暂停、倍速切换、音量调节、播放进度条的拖拽同步，以及空格键控制播放状态等。随着播放页面基础逻辑趋于完整，本节课开始进一步实现视频播放中非常重要的交互功能——**弹幕系统**。

弹幕功能在主流视频网站（如 B 站、优酷等）中已经非常成熟，用户能够在观看视频的过程中实时发表观点，评论会以“从屏幕一侧飞向另一侧”的形式动态展示，从而营造更强的互动氛围。为了实现这一效果，我们需要同时处理界面布局、弹幕显示逻辑、播放时间同步以及样式区分等多个方面。

**1. 弹幕显示的基本要求**

根据功能设计，视频中的弹幕需要满足以下规则：

1. **默认展示三行弹幕**
    视频的某个时间点可能有多条弹幕，为避免重叠，我们提供三条独立通道进行排列显示。
2. **按发送时间展示在视频的对应位置**
    比如某条弹幕是在 *1 分 30 秒* 发送的，那么播放到该位置时这条弹幕应从右侧进入并开始滚动。
3. **同一时间点多条弹幕的布局方式**
   - 第一条 → 第一行
   - 第二条 → 第二行
   - 第三条 → 第三行，但需 **整体向右缩进两个汉字**
   - 同一行内的多条弹幕之间需要 **间隔 4 个汉字的距离**
4. **弹幕不允许换行**
    即使文字过长，也必须在单行内滑动显示，不能自动折行。
5. **自己的弹幕需要与其他人的样式有所区分**
    区分方式可以是字体颜色不同，或者加入用户头像等。

这些要求共同构成了接下来布局和逻辑处理的基础。

**2. 弹幕区域的整体结构**

课堂上老师强调，弹幕区域并不直接绘制在视频播放器的内部，而是通过创建一个 **独立的 QDialog 对话框** 叠加到播放窗口上：

- 对话框 **无边框（Frameless）**
- **背景透明（Translucent）**，保证底层视频可见
- 内部分为三行，每行是一个独立的 **QFrame**

这样的设计让弹幕系统成为一个在视频层之上的“透明覆盖层”，既不影响视频渲染，又能灵活控制每一条弹幕的布局和动画。
...
接下来我们结合课堂解释，对核心逻辑进行讲解：

**(1) 创建透明无边框对话框**

老师强调过，这个对话框是整个弹幕系统的载体，“看似不存在，但其实悬浮在视频画面上”。通过 `FramelessWindowHint` 和 `WA_TranslucentBackground` 实现：

- 没有标题栏
- 没有可见的边框
- 背景透明，视频可透过它显示

这样，即使弹幕覆盖在上面，也不会挡住视频内容。

**(2) 三行布局器的构建**

使用 `QVBoxLayout` 垂直排列三个 `QFrame`：

- top（第一行）
- middle（第二行）
- bottom（第三行）

每行高度固定为 38 像素，布局边距与间距都设为 0，让三行严格贴合对话框大小。

老师在课上特别强调，这种纯代码布局方式能保持结构更清晰，也便于后续加入动画控件。

**(3) 弹幕区域的定位**

课堂上提到：“弹幕窗口必须放在播放窗口的 head 下方”，因此代码中通过 `mapToGlobal()` 获取播放窗口的绝对位置，再根据播放头的高度偏移，确保透明对话框恰好覆盖在视频画面区域。

**4. 封装单条弹幕元素：BulletScreenItem 的设计与实现**

在上一节我们完成了弹幕区域整体结构的搭建，包括透明悬浮的弹幕对话框以及三行弹幕的布局。
 从这一节开始，我们进一步深入实现弹幕系统的核心组件——**单条弹幕的封装类 BulletScreenItem**。

在真实的视频网站中，一条弹幕可能是普通文本，也可能来自当前用户并携带用户头像。为了统一管理不同类型的弹幕，同时便于后续进行动画处理和布局控制，我们需要将“弹幕元素”抽象为一个独立的 UI 组件。因此，这里采用 Qt 的方式，创建一个继承自 **QFrame** 的类，用以封装：
...
这一封装是后续动画、时间同步、文本长度计算等操作的基础。

**(1) BulletScreenItem 类的结构设计思路**

老师在课堂中解释时强调：**之所以要封装一个类，而不是直接把 QLabel 塞到弹幕区域中，是为了让每条弹幕具备可扩展性。**
 例如后面我们需要实现：
...
- 弹幕需要独立的动画
- 弹幕可能需要自适应宽度或动态背景
- 同一行的弹幕需要计算自己的宽度与前一条保持间隔

这些功能都需要统一的“弹幕对象”，因此继承自 QFrame 是最便捷的方式。

**(2) 核心代码解析：布局、文本与头像封装**

下面是板书中给出的 BulletScreenItem 源码，我们在此基础上分析其功能设计：
...
每条弹幕由一个固定高度的 QFrame 承载：

```cpp
setFixedHeight(38);
```

这与之前在弹幕区域中设置的三行高度一致，因此 BulletScreenItem 会完美地嵌入 top/middle/bottom 三个通道。

水平布局（QHBoxLayout）管理头像与文本

课堂上老师解释：弹幕最核心的视觉结构就是“头像 + 文本”横向排列，因此使用 QHBoxLayout 是最自然的方式：
...
```cpp
textLabel->setStyleSheet("font-family:微软雅黑; font-size:18px; color:#FFFFFF;");
```

字体大小和颜色都为常规弹幕样式所使用，`adjustSize()` 的作用是让文本根据实际内容自动扩展宽度，方便后续计算动画起点和终点。

**(4) 头像的显示与用户弹幕的样式区分**

对于“当前用户发送的弹幕”，系统需要在前面显示头像并加外框，以便区分。
...
只有调用 `setBulletScreenIcon()` 时才展示头像。

2. 设置头像并添加外边框

```cpp
this->setStyleSheet("#QFrame{ border:1px solid #3ECEFE; border-radius:19px; }");
```

老师在课堂中提到，蓝色边框代表“自己的弹幕”，这样即使没有头像也可被识别，但有头像时效果更明显。

边框圆角为 19 像素刚好使整个 QFrame 呈现圆角矩形，与头像尺寸协调。

3. 头像缩放模式

头像大小固定 30×30，并设置 `setScaledContents(true)`，确保无论头像原图大小如何，都能自适应显示。
...
`adjustSize()` 非常关键，它的作用是让弹幕宽度根据文本的实际像素宽度自适应。

这一点对后面几件事情都非常重要：

1. **计算弹幕整体宽度**
2. **保证同一行的弹幕之间始终保持“4个汉字”的间隔**
3. **动画起点位置 = 视频宽度 + 弹幕宽度**
4. **动画终点 = -弹幕宽度**

课堂上老师反复强调：

> 弹幕宽度不能写死，必须动态调整，否则长文本会被截断，也会导致动画走位偏移。

**5. 为弹幕添加动画效果**

在上一节中，我们已经完成了单条弹幕元素 `BulletScreenItem` 的封装，实现了文本显示、可选的用户头像展示，以及基本的布局结构。这样，当我们拿到一条具体的弹幕数据时，就可以直接通过该类将其显示到界面上。然而，真正的弹幕并不仅仅是静态地呈现在屏幕某一位置，而是要在视频播放过程中“飘”过屏幕，从右侧滑入，然后从左侧滑出。因此，本节重点是学习如何为弹幕添加动画效果，让其实现横向移动的动态表现。

Qt 为我们提供了一个强大的属性动画类 —— `QPropertyAnimation`，它可以让某个控件的某项属性随着时间平滑变化。属性的内容可以是控件的位置（pos）、尺寸（size）、透明度（windowOpacity）或颜色等。我们能够通过在动画中不断改变控件的 `pos`（位置）属性，让弹幕不停向左移动，从而实现经典的弹幕飘动效果。

`QPropertyAnimation` 的构造函数接收三个参数：动画目标对象、目标属性名称，以及可选的父对象。目标对象非常容易理解，就是需要执行动画的控件；目标属性则告诉动画系统我们希望改变控件的哪项属性；而父对象的设置关系到动画对象是否加入对象树中，并决定其释放方式。在本次弹幕中，我们需要修改的就是位置属性，因此在创建动画对象时会将属性名称设置为 `"pos"`。

拥有动画对象后，需要对其进行一系列配置，例如设置动画持续时间、确定起始和结束位置等。如果不手动设置持续时间，默认只有 250 毫秒，对于弹幕来说明显过短，因此需要自行调整。动画的起点是弹幕一开始出现的位置，通常是在屏幕右侧某个位置；动画的终点则是将弹幕移出屏幕左侧，因此可以用 `0 - this->width()` 作为其终点坐标。位置相关的数据类型是 `QPoint`，而由于 `QVariant` 内部支持自动类型转换，我们只需要在`setStartValue()` 和 `setEndValue()` 中直接提供 `QPoint` 即可。

为了确保动画结束后能够进行清理工作，我们还需要连接动画的 `finished` 信号。当动画播放完毕时，会触发这个信号，我们就可以在槽函数中将当前的弹幕对象销毁。这样既避免遗留无用控件，也确保界面不会持续堆积无效的弹幕项。
...
在构造动画时，我们将动画作用于当前的弹幕项，并将其属性锁定为 `"pos"`。随后设置持续时间 `duration` 并给定起始与结束位置。起始位置的 `x` 值通常由外部传递进来，用于控制每条弹幕的初始显示点（例如让不同的弹幕位于不同轨道上）。动画结束后，槽函数会自动释放该弹幕控件本身。

启动动画时，只需要调用 `startAnimal()` 方法，其中 `show()` 是为了确保控件在启动动画前已经显示，随后调用 `animation->start()` 即可让弹幕开始飘动。
...
当⽤⼾选择对应的分类时，该分类下的标签应该以按钮的形式显⽰在界⾯上。因此需要添加 addTagsByKind(...)函数，实现标签按钮的动态创建。
...
接下来我们将分类对应的标签添加到上传视频的页面中。在添加之前，我们先了解一下大概的操作流程。首先打开我们的工具，选择我的视频来到我的页面，点击上传视频的按钮。点击之后，我们选择要上传的视频路径。我们寻找一个视频上传，点击进去打开后，页面会切换到上传视频的页面。上传视频的页面完成后，请注意刚开始标签位置是空的，我会截图给它添加一个分隔线，变成黑色粘贴过来。这个位置大家需要注意，改成红颜色。刚切换到视频上传页面时，我们的分类没有选中。在分类过程中，没有选中我们提供的标签，底下也没有任何标签按钮，这是正常的。因为分类没有，所以标签在这上肯定也没用。完成之后，用户可以选择对应的分类。我们在查看qcom和box中的分类变化之后需要更新标签下的按钮，这些都是理所相当，我们都可以想象。例如我们随意为他选择一个旅行程序，当我们已经在内部栏中选中的文本发生改变时绑定槽函数，槽函数会在内部进行调用。触发后，我们可以在槽函数中添加对应标签和按锁定按钮。选择完成后，点击提交。点击提交后，我们再次进行演练。假设下次我再上传视频时，选中一个视频路径，然后打开。打开后它是否又回到了我们的上传视频页面？回来后，需要注意分类和标签，并清空。如果我们没有清空，他留的仍然是上次选择的数据。选择数据是指当用户再次提交或者上传视频时，我们需要将上一次遗留的内容上传到视频中。我们需要清空界面中遗留的数据。清空之后是分类，而不是分类。我们将Qqcom com box设置为选择优势，当前选择的我们将其改为-1即可。我们可以将标签设置为空，标签设置为空后，我们无法将分类设置为空。我们等待分类设置为空。之后我们无法在这个地方获取到任何标签。既然标签无法获取，那么在这个位置上，我们不会在界面中添加标签按钮。添加标签的操作需要封装成函数。这个函数是add，将其转换为泰戈兹，未来需要通过标签分类进行添加。插入后修改为抗死QS准，之后查看即可。如果用户选择了具体的分类，那么我们将分类栏上传。当我们上传完成后，用户下次再次进行上传时，我们会先将这个地方的分类设置为空，在里面设置为真空。这样就相当于清空界面中分类底下的标签按钮。这样界面中标签位置就恢复成功，之后就将它恢复到原来的初始状态。之所以在里面添加这个方法，是因为设置空的这一步操作在信号给槽函数中无法进行控制。因为槽函数无法直接调用，所以只能自己封装函数并添加标签。添加这些标签，将来我们自己封装的函数放到Com box，让它的当前文本发生改变，在槽函数中调用即可。接下来我们需要为其封装一个函数，这个函数的pravate是私有的，不需要在其他位置上进行调用，返回者没有这个功能。我们需要通过分类添加这些tags。我们将house故障修改为q斯顿，并将其传递进来。传递完成后，再让它继续前行。同志们注意这个位置，我们为他添加奥特加回车，让Qzone代替我们在源文件中生成方法的定义。关于方法的定义，我们将这个方法直接在我们刚才添加的类当中进行调整，你不需要打印，直接调整我们对应的添加方法即可。根据我们刚才讨论的添加方法，在添加方法中，我们首先需要清除之前的数据。我们将之前的数据清除之后再获取当前分类对应的标签。获取到这些标签之后，第三步添加标签按钮，这步标签按钮到我们的layout中就可以了。我们将其添加进来大概分为三个步骤。第一步骤我们可以先将其保留。我们从第二步骤开始，稍后让程序执行并且将标签分类切换，这样大家就可以看到我们在分类中添加了哪些原理，一些标签按钮就会累积进去。稍后我们再观察它的效果，直接从第二步开始。第二步，我们需要获取标签，这个位置我们需要拿到对他身份的指针，这是针对**生活的指针。在Model类中，我们有贝塔色谱的类。内部有静态的主要方法是get use，拿到生成的指针后，再给out看安的AMD格子。探戈皮甲吗？你可以调整get标签和分类指针，获取指针之后我们便可以获取所有标签，这一规则就是所有标签。我查看获取它的所有标签，这是坦克斯，他可以了解我们如何处理这件事情。首先我们需要查看安得皮卡。请你在里面调整，我们那边有一个看得坦克斯and白看到把这个看到给它传进来，传进来之后我们拿到返回正常是Q吗？我们获取q希的结构，q希的结果底部再给他上一个位置的k字，我们便可以获取所有的k字，这样我们便可以获取当前分类底下的所有标签。这个位置还有一个问题，我们刚才提到，如果我们把界面分类，下次再上传时，需要将界面中的元素清空。如果清空，那么我们需要将com box的当前选中设置为-1。设置成-1之后，这个位置的分类没有，分类在里面是一个空的。如果分类是空，那么我们就无法获取任何标签数据。因此这个位置需要进行检测。如果我们提供的分类在里面是is empty，那么分类在里面是空的，我们的程序直接让它返回即可。我们不需要获取这些标签。我们把这些标签拿到之后，我所需要做的操作是遍历并且发生变化。我们将负循环导入A，由求凹凸，然后在x中进行遍历即可。这样就可以获取每个标签。获取标签之后，我们需要为他人创建按钮Q18特，并查看btn。我们在btn右侧扭动铺18层，并且将副元素放入其中。Y离子即坦克外离子，我们将复元素改为UI点的泰安哥where did，这样复元素便设置完毕。设置完毕之后，我们还需要设置按钮，它的大小是多少？我们查看btn B签字之后，赛特将fixed size改为90，高度改为49。我们修改宽度和标签之后再设置文本。文本是我们刚才获取到的标签，这个按钮在里面，我们将其点击选中和未选中进行处理。设置按钮在里面的状态，Tag btn在里面，即塞尔特号的设选是否选中的状态，你能否被选中？我们将其改为一个处。代码锁的作用是设置按钮状态，目前有两种状态分别是选中和未选中。将来我们先进行分类，它会弹出一堆标签按钮，我们需要为它选择几个作为当前视频的标签。你选择了几个，我不知道你的按钮是否已经选中这个位置，我们将这个按钮设置为可选中的。我们可选中的两种状态在这个位置是q图10，必有其中。如果他现在状态是check ed，如果check走，表明当前按钮被选中。如果q不是必由TT，状态在这个位置是on ch一，是CK，on chat，说明当前按钮未选中。这一部分没有选中，主要是我们需要通过选中和未选中设置按钮上的优势。选中和未选中的背景颜色不同，将来提交时，我们需要知道哪些按钮选中了，哪些按钮没有选中，并将其上传到服务器。这个状态需要设置，位置设置完成后再让它往下走，我们将按钮的样式设置给对方。第二是赛特斯丹欧shit Style shit，这是我们给的q忽视，必须有机器。我们需要为这个按钮设置样式。在拉过来后，我们会加上按钮的边框，包子改成线宽和像素，然后在里面通过bug实现。我们将线的颜色改为井号三，ece三e三ece三ece，Fe会给出这样的颜色，这个颜色我们之前用过，是蓝色的。三、ecefe边框给完之后，我们再给它添加上一个包的杠radius和圆角半径，这个圆角半径改成4个像素点即可，总共444个像素。接下来是按钮上的文字颜色，我们将其改为三、e cefe。三necefe继续前进，还包括字体大小，这是方特size，包括字体类型。F am方特family，我们将其改为微软雅黑。我们到方车杠赛，我是否放声？将其放在塞子上，改为16号即可。样式尚未完成，按钮上的样式较多。大家将葛朗的杠Col班格朗达卡改为井号f，背景将其改为白色。我们将按钮改为白色，这样我们就设置完成按钮。设置完成之后我们需要处理按钮选中和未选中的样式。Q铺是b，有7gon。如果我们的按钮处于选中状态，就是checked，checked里面就是选中状态。如果选中了，我们就需要在这个上面处理背景颜色。Background color，我们将背景颜色改成井号cnecefe。它有自己的颜色，将字体颜色改成井号f，字体颜色在里面改成白色。我们已经将其选中状态处理完毕。处理完毕之后还有未选中的状态。请注意未选中的状态，嘲笑未选择的状态，q不是b1TT，基本上是Checked。如果未选中，那么beckground大家乱写，如果让c ol背景颜色改变，那么我们将其改为井号f，即全f。之后需要设置COO文字的颜色，我们将文字的颜色改为井号，即三ece fe，即31cfe。我们已经设置按钮的样式，最后需要将按钮添加到布局器中。布局器内容只能查看物品，UI上面是探戈料子，我们需要外力将第一天必签的按钮添加进去，这样我们就可以将所有按钮添加进去。目前我们已经将分类对应的所有标签拿到手中，对于每个标签我们创建了一个按钮，并将按钮添加到水平布局器中。然而这个位置上仍然存在一个小问题。请查看我们的板书，假设这是布局器，我们为它创建了一些按钮，这些按钮的宽度并不能够充分满足我们整个布局。在未充满的情况下，所有按钮将来会均分整个布局的宽度，即在我们的布局中采用均分的方式进行布局。这一点不太符合常规，而我们常规需要让这些按钮尽量往左侧靠。我们需要在布局器中添加水平弹簧或者空白间距，将按钮挤到左侧，尽量让它向左侧靠拢。添加弹簧和水平空白在里面都可以。我们需要在泰安格勒ot类out中，Lay out到最后一个按钮的右侧添加空白间距或者使用水平弹簧将这些按钮挤到左侧。违章如何处理？我们先拿到固定器，泰安哥的料子在布局期中有一个方法，影子的特色。微信银行插入space是间距，我们给它插入间距，这个方法有两个参数，一个是需要把水平间距插在哪个位置，插在哪个位置就是一个索引。第二个参数是水平宽度，即未来需要多宽。索引位置较简单，我们想要将其放置在最后位置，需要了解布局期中的元素，即按钮的个数。因为它从0开始，所以我将所有按钮设置为size，恰好在最后一个按钮之后进行插入。插入的第二个参数是间距，实际上我们只需要减去所有按钮的宽度以及按钮与按钮之间的间距即可。我们先找到它的副元素，即泰国外地的宽度，用宽度减去所有按钮。我们之前设置的固定宽度是9898，关于按钮的宽度，稍后我们可以通过料子的space进行设置。设置按钮与按钮之间的间距为20，我们总共有多少个按钮，这个位置可能是开泰安格斯点的size，比安格斯点的size。我们在最后一个按钮之后，将空白距离间距插入好，插入好之后不要忘记这是泰安格勒的样子。我们将Site、space设置间距，即按钮与按钮之间的间距。我们将20改为20，完成后将代码编译，让程序运行。我们查看最终效果，目前程序正在编译，请耐心等待。程序启动后，分类位置没有进行任何选中，标签底下默认为空。当选择分类改变后，标签按钮才能显示。请随便点击我的。点击上传视频，让它向回退一步，在videos目录选择我们要上传的视频，这样就会跳转到上传视频的页面。之后我们的分类没有选中，分类是空的，标签也没有。例如我随便选择一个分类科技，这样我们的按钮已经想象出来了，但是想象出来之后我们发现它的样式可能不对。当按钮被选中后，我们发现并未看到对应的边框。我们查看这个位置，它包含一个像素，这个是否写错了？它是一个实现，即将solid改为solid，并非我们所包含，稍后我们再编程序运行起来就可以看到效果。每个按钮背景颜色是白色，字体颜色是我们现在提供的这种字体吗？字体按钮与按钮之间的间距是20，只是边框没有显示，我们无法看到它们之间的间距。这些按钮并未区分我们的太阳格外地毯空间，而是在右侧插入了一个空白间距。这是科技。如果我再选择一个动物，那么我们发现动物的按钮标签和我们之前的科技按钮标签是否全部在料子中存在？因此我们需要在添加新按钮元素之前清除上次留下的元素。我们需要清除进入之前的数据，首先需要获取这些按钮，获取到之后从外地图中移除即可。我们接下来将获取上一个y获取的料子，即layo lay路由器layout中的元素，将其删除。我们可以通过cute提供的方案出展方法获取内容。所有按钮都是太阳哥外地图的子元素，可以让他调用find求准的方法发出转，找一批。这是一个泛型方法，我们需要找到这些孩子的类型。现在我们要找到的是它内部所有按钮，铺18层给一个星号，翻车准确后，他找到的孩子将来会以Q list为基础，Q利斯是Q服务师巴特的新号，它会返回这些按钮。这是我们的tag btn例子淘宝仓。我们拿到这些按钮后，需要遍历凹凸泰尔格b7n并且便利tag一天List。我们将当前按钮从布局器中移除，它的操作是UI找到我们的探戈料子。探戈料子的布局器，给他上一个蕊木屋外力套，把这些元素移除掉，直接移除碳和btn，这个位置大家需要注意。我们提供的蕊木r一MV一w ID这一切仅是将元素从布局器中移除。它并不会真正的删除我们的这些按钮，因此我们把它移出来之后还需要自己手动添加，这个规则是你自己手动移除的，所以需要我们自己手动进行删除。手动删除我们之前的按钮都是由它弄出来的，手动删除直接用data将tag1000删除掉。在删除它之后，二就被删除完毕。最后我们还为它添加了一个空白间距，这也需要将其删除。底部的味道应该在上面，你刚才在上面删除了按钮。底部需要获取空白间距并将其删除。如何在里面获取？因为在布局系中，我们寻找探戈料子，找到它后，内部有一个艾特方法，即用于获取我当中的某一项。某一项的反馈作用是的艾特。新拿到这一项后，你就可以对它进行操作。这个空间是否位于我的最后一个元素的位置？因此我们可以通过layout实现。坦克里奥特内部有Count的方法，Count的方法是获取内部元素的个数。我们已经删除这些按钮，现在它内部只有一个空白间隔空白，在布局期中索引自然为0，而这个位置的个数为一，因为只有一个元素，所以我们现在这个方法需要一个下标，给它减一个一即可。刚才看到这个方法返回的是qa of Lay out。At，我们给他一个星号即可。Spasesvs at，我们拿到空白检验卷之后再让它往下走，底下有UI六探戈六，让他为木屋I疼，将Space直接删除。删除之后，我们刚才这部分所给到操作，第一次添加进去的是科技所并的标签以及空白间距。当我们第二次勾选动物时，所写的阿德塔x班方法又会再次调用。我们会将前面的元素删除。删除之后，我重新获取当前分类所对应的标签，并用每个标签构建对应的按钮，将按钮添加到工具箱中，空白添加第二次就不会有上次残留的数据了。修改完成后，我们再让程序运行，观察它的效果。刚才这个位置的按钮没有那些空白。按钮没有空白，也没有扫包。之前的规则写错了，它只有一个像素，第二个参数代表包的线是实线、虚线、点线，还是让程序运行。运行之后，随便点击封面，点错了，应该是选择我的进来后上传视频，让他往回退再往回退，找到videos把视频选中打开。我们打开之后可以为它选择分类，例如我们随意选择美食，这些标签已经出现。完成之后我们再为它选择运动，运动后所对应的标签也会出现。之前的标签不会再残留在我们的窗口中。现在这个按钮有两种状态选中和未选中，例如我给它选中汪星人，可以发现它的颜色发生了改变。如果让我们选中了宠物，那么你可以选味道。例如我发现萌宠在哪里选错了，我再点一下它又恢复到原先的状态。这样我们就可以将上传页面当中的分类和标签处理完成。
...
我们项目中的前端界面部分数据模型已经在前段时间的学习中采用qt进行界面开发。例如我们对界面布局已经有初步了解，如何在Q7中添加UI界面，如何让Q7的设计师通过拖拽空间的方式设计界面，包括自定义界面和类型提升等。我相信大家在后续工作中设计一个简单的页面，将其与我们具体的信号槽绑定好，处理逻辑操作基本没有太大问题。接下来我们将讨论项目中的业务逻辑部分。这些业务逻辑部分仅依赖客户端无法完成，因此我们需要搭建自己的测试服务器配合客户端开发，完成后续业务逻辑操作。在进行网络处理之前，我们需要为大家简要介绍项目开发流程。目前我们的客户端没有灵魂或者它的开发仅是一种界面展示。虽然它已经具备播放视频发送弹幕的简单播放控制功能，但是它仅能播放客户端的视频，或者将其视为本地视频播放器进行对待，这是可行的。我们将其视为播放平台的界面进行客户端处理，然而这远远不足。我们尚未处理界面中的业务逻辑操作，仅是界面图展示。例如，首页在点击分类或者标签后，我们想要获取视频信息，然而我们需要了解视频信息的位置和所在位置。我们必须在服务器上构造出一个用于测试的服务器，并将视频数据和假数据放入其中，让客户端连接服务器，获取服务器上的视频数据。例如，我们需要完成登录注册功能，在进行注册时，你需要向谁注册以及将用户的注册信息提交给谁，如何保存信息？在登录时，我们需要了解当前用户是否存在等信息，因此客户需要进行后续处理。为了让客户端能够完成具体功能，我们需要对后续业务逻辑代码进行处理。在整个处理过程中，我们需要搭配一个服务器，这个服务器只是我们自己用来测试的服务器，与真正项目的服务器是两回事。这是两件事情。我们搭建的服务器仅用于客户端与服务器之间的交互，以完全实现业务流程的运行。要搭建这个服务器，需要借助kot提供的网络模块，搭建网络客户端和网络方面的服务器。接下来的工作是我们开发客户端，客户端的业务逻辑操作时，之前的界面部分布局不涉及业务逻辑，它仅是一个纯客户端的内容。一旦涉及业务逻辑部分，我们就需要添油器，使用添油器提供网络相关模块，搭建网络客户端。这是搭建一个用于测试的服务器，刚才提到的服务器仅配合我们进行功能测试。它并非真正完整可用的服务器，通常情况下我们将用于测试的服务器称为曼克斯网mo CK，m股CK f一V，这个梦可能是假的意思。我们将来会为其构造一些虚假数据，使客户端和服务器之间的流程能够顺畅地进行。我们提供的测试服务器的作用是进行客户端的内部测试，让Monkey server配合我们完成客户端业务逻辑处理。我们在后续开发过程中既要完成客户端操作，也要完成马克思网中的操作。例如我们刚才提到的登录注册，当用户将自己的信息填到界面上时，客户端需要先拿下这些信息。将来马克思网络接收到客户端发送的登录请求后，需要响应我们，我们可以制造一些假的数据。制造虚假数据的过程大致如此。我们继续讨论马克思王的问题。我大致了解了它的含义，以及我们为何要搭建mock monk server的服务器。请注意，在我们公司的开发过程中，客户端和服务端并非一个人完成。公司内部的企业级项目较大，如果让一个人完成，那么一个人可能需要三五年的时间，当项目落地时，市场先机可能就会失去。因此一个项目的开发通常由前端和服务端的两个团队共同完成。这两个团队在开发前需要做一些前期准备工作，例如项目需求、技术选型以及前后端接口设计约定等。等到前期准备工作铺垫完成后，才进入开发阶段。在开发过程中，你负责前端界面，我们负责服务器端。两个团队的工作是分开进行的吗？大家各自开发各自的模块，等前端部分和后端部分都开发完成之后，我们再将前端和后端连接到一起，进行共同联合调试。检查前端接口是否存在问题，后端提供的响应是否有问题。两端共同通讯的过程，我们称之为联调。我们的项目在联调通过之后需要进行整体测试。虽然技术人员和开发人员在整个流程完成后没有问题，但是并不意味着项目中没有问题。项目需要交给测试同事，让测试同事对整个项目进行完整测试。测试完成后，所有bug已经修复，整个项目才算开发完成。开发完成之后，客户端和服务器两个团队通常采用并行开发模式，并行开发独立的客户端。在客户端开发时，服务端的代码也在编写。因为客户端开发到后期需要搭建测试服务器，所以将来也需要搭建测试客户端或者借助其他第三方工具完成接口测试。客户端开发在公司的实际开发中通常由两个不同的团队开发，这两个团队将来分别开发自己的模块。在开发完成或者调试没有问题之后，我们将前端和后端的代码放在一起进行联调和测试调试，之后将这个过程称为联调。客户端完成联调后，你可以自行检测服务端的代码，我们自测就没有问题了。接下来将前后端整合到一起，观察业务逻辑，并且确定客户端提供的请求服务如何响应。这个过程称为连桥，等连桥和联调通过后，再将项目交由我们。这个项目交由测试同事进行测试，完成后bug已经完全修复，这个项目才可以部署。部署上线后，这个项目才算真正开发完成。在这个过程中，按照我们刚才提到的客户端和服务器的代码，两个团队是独立开发的，并行开发。你开发你的，我开发我的，可以下来吗？客户端开发时，我们的服务端代码仍然在编写，而我们的服务端开发时，客户端的代码同时也在编写。因此我们需要借助刚才提到的Mark服务器配合客户端进行开发。在这个位置，我们提供的服务器、测试服务器和客户端的功能并不完整。按照我们刚才讨论的，客户端和服务器的代码是独立开发的，对于服务端客户端，将来需要自己搭建测试服务器。通常我们将其称之为万科思网。对于服务端，它将来需要搭建自己的测试客户端。测试客户端可能只是书写一个简单的页面，用于调用后端的接口，或者直接使用第三方工具，例如波斯特曼等测试接口。大家需要注意，我们提供的测试服务器和客户端功能并不完善，他们只是用于配合，功能简化。许多业务逻辑已经被省略，仅需构造一些假数据配合另一端进行测试。例如当前我们在进行客户端开发时，需要构建一个测试服务器max，我们不需要真正处理它的业务逻辑，只需要按照前端约定好的接口实现一个简单轻量化的服务器搭配测试即可。我们需要注意此位置的测试服务器和测试客户端。测试客户端的功能并不完善，很多业务逻辑已经被经典、精简或者省略。这部分内容被省略，我们只需要构造虚假的数据。只要能够通过一些虚拟数据完成另一端的测试工作即可，你就能够帮助我们的后端实现流程的畅通。我们前期要向大家介绍福田知识，即对于客户端，接下来我们需要搭建一个测试服务器，或者我们可以直接将其称为Mark server。在搭建马克思路时，我们面临一个问题。例如我们需要获取5个分类下的视频，那么我们有两个请求。请求中的内容如何填写？我们在进行网络通信时，如何提供网络协议？如何沟通和确定？未来视频数据在网络中传输时，我们如何实现序列化？当服务端收到请求后，它如何响应请求并填写数据？你在前后端进行交互时，必须有接口约定，以便获取分类下的视频请求。服务端如何响应？我们需要提前约定好响应方式，例如前后端交互接口的约定。如果我们没有这些约定，那么你在开发客户端时可以自己定义前后端交互的结合。我们服务端在开发时，将来也会自己定义前后端约定的结果。你们两个开发完成后，放在一起进行联销时，谁都不知道对方给到响应或者请求的具体样式，程序依然无法运行。因此我们需要提前约定好工作量。我们刚才在这个位置遇到了一个问题，即前后端交互接口的约定。我们需要了解你刚才提到的位置约定，这个约定我们平时在生活中一直在谈，例如我和谁约定去某个地方玩或者见面等。我们突然使用项目开发中的专业术语提出一个前后端交互接口约定，这个约定是一份协议，我们需要双方沟通好。等到这份协议规范确定之后，大家都按照规范执行，才能保证整个项目的顺利开发和前后端的正常通信。例如我们的电脑上都有USB接口，未来我们可以在USB接口上插入鼠标、键盘或者U盘等USB设备。电脑生产厂商在生产电脑时是否需要直接与键盘鼠标USB厂商沟通，确定USB接口的大小、宽窄以及上面有几个USB引脚和电压大小等问题。我们不需要约定这些事情。在未来开发时，电脑生产厂商和USB设计生产厂商只需遵循同一套USB协议，这个位置的协议就是约定。只要大家遵循同一份约定，未来我们开发的两套设备就能够进行通讯和交互。根据我们刚才的分析，你所提到的协议，我大致明白了你所说的约定，实际上它是一份协议。首先是标准规范，我们制定完成规范之后，大家都按照规范约定执行，这样才能保证设备或者程序正常通信。我们将约定明确之后，刚才提到的前后端接口约定自然而然地清晰。前后端接口的约定是为了完成某种逻辑操作，客户端和服务器之间需要进行通信，必须将一些约定好的事情确定下来。前后端接口的约定通常是一份文档。通常是一份文档，文档中约定接口的实现。我们刚才提到细节，并非在拿到项目之后立即编写代码实现。在实现之前，还有许多事情需要完成，比如市场调研、需求分析和技术选型等。在完成之后，还需要确定项目的接口设计等。整个过程需要前后端相关人员不断开会和沟通，才能确定最终接口文档的约定。在完成前期准备工作后，才能进行开发。我们需要提前约定好前后端接口的时间和实现细节。例如，客户端服务端采用何种网络协议，采用Http、https还是FTP。如果数据在网络中传输，我们必须采用序列化和反序列化的形式。如何适用介质进行处理，是采用XL还是普罗托巴法等方法？客户端发送的请求服务端应该如何响应？服务端在响应时应该包含哪些数据？这些都是在项目开发之前约定好的或者在前后端约定的接口文档中规定好的吗？我们在位置上会约定哪些东西，例如客户端和服务器。服务器在通讯时使用的网络协议，我们这个位置所说的协议，是指我们应用层的协议吗？应用层协议包括序列化，这似乎是反序列化的方式。客户端有哪些请求以及我们提供的服务器如何进行响应等，这些都是在开发之前约定好的内容。例如在项目之前，我们负责后端工作。我们主要负责客户端界面。后端由比特的张文超老师和超哥完成。在开发这个项目之前，因为只有我们两个人在参与开发，所以我们需要提前约定好接口信息。我们约定好在约定时，例如客户端需要获取错误信息，或者在用户注册或者使用手机号登录时，需要填写短信验证码。另外填写短信验证码，用户点击获取验证码按钮后，客户端就能够获取手机号。我们获取客户端的手机号之后需要向客户端发送请求，请求中的URL、数据以及客户端收到请求之后如何响应。我们只需要约定这套机制，例如文章中提到的第二点是获取短信验证码，这个接口的约定非常清晰，我们将来会采用Jason的方式Restful风格处理这份约定。关于rest付款，我会后续为大家进行详细介绍。从当前数据中，我们在进行通讯时使用的是应用层的HTTP协议。请求方法在boost方法中，Url网在其中。HP server目录下有一个干透接口，它有一个扣子请求。服务端有应该扣除的服务，我们需要发起负责的请求，请求中需要包含哪些数据，例如request、ID筛选ID以及电话号码，为什么需要快速ID筛选ID？后续我们具体开发时再向大家介绍。总之，获取验证码时必须拿到手机号码。我们收到请求后，将请求通过网络交给服务器。服务器收到请求后，他们清楚客户端需要验证码，他们会通过其他手段和机制生成一份短信验证码。验证码生成完成后，系统会进行响应。当服务端收到分时响应后，它会知道我刚才发送的获取验证码请求已经被服务器处理。这样前端和后端的交互就完成了。未来在各自前后端开发代码时，我们就不需要为接口的约定而烦恼。客户端在开发时，我们只需要将前端请求监管处理好。将来在not生活中，我们并不需要真正开发短信服务模块，只需要模拟在Mark server中，将响应返回，并在返回响应中附带短信验证码即可。是吗？在服务端开发短信验证码时，需要借助第三方短信服务，为手机真正发送短信。短信中包含短信验证码，需要完成整个流程。对于Mark搜房，我们只需构造一份假数据即可。我们已经有接口文档的约定，大家各自开发代码并行处理没有任何问题。我们在解决开发基本知识后，需要搭建本次项目中的网络客户端以及曼克斯网络。搭建完成后，我们再进行后续业务逻辑处理。
