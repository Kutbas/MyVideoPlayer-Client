...
新建一个qt设计师界面，命名为AdminWidget，将AdminWidget的geometry的宽高修改为
1350*792。然后按照以下步骤进行布局：
拖拽一个QWidget到AdminWidget中，objectName命名为backgroundWidget，背景色设置成
rgb(170,170,127)，然后选中AdminWidget点击水平布局，将AdminWidget的Margin和Spacing均设
置为0
②拖拽一个QWidget到backgroundWidget中，objectName命名为container，背景色设置成
rgb(255,255,255)，然后选中backgroundWidget点击水平布局，将backgroundWidget的
BottomMargin设置为5(修改15)，其余Margin设置为16，Spacing,设置为0
③拖拽一个QFrame和QWidget到container中上下摆放，QFrame在QWidget上方，objectName从上
往下依次修改为btnBox和pageBox，将QFrame的背景色设置为rgb(170,255,255)，将QWidget的背
景色设置为rgb(170,170,255)，然后选中container点击垂直布局，将container的TopMargin和
BottomMargin设置为20(修改o)，LeftMargin和RightMargin设置为24，SPacing设置为15，将
btnBox的minimumSize和maximumSize的高设置为60
④拖拽两个QPushButton到btnBox中水平摆放，objectName从左往右依次修改为checkBtn和
roleBtn，checkBtn的文本修改为"审核管理”，roleBtn的文本修改为"角色管理"；拖拽一个水平弹簧
放在roleBtn之后，然后选中btnBox点击水平布局，将btnBox的Margin和SPacing全设置为0，将
checkBtn和roleBtn的minimumSize和maximumSize的宽高均设置为1o0*60
拖拽两个QWidget到container中，objectName修改为checkTable和roleTable，将checkTable的
背景色修改为rgb(255,170,127)，geometry修改为[(0,0),1270＊686]，将roleTable的背景色修改
为，geometry修改为[(0,0,),1270＊ 686]
...
你老婆知道，对于系统页面的主框架有了简单的了解之后，接下来我们可以将系统的主页面的框架把它先设计出来。Ok它的实现步骤同样我已经在我们的文章当中罗列出来，接下来我们就直接按照之前我所罗列好的步骤，一步一步往下来进行操作就可以了。好，我先把它拷贝过来。在操作之前，同样我们先给它创建一个分支对吧？我们不要在顶外的分支底下进行任何的操作来，然后右键单击创建分支的话，在传统页面当中跟二级菜单里面我们去选择switch开票或者说可以增加都可以对吧？可以分割，它只是单纯的把分支创建好了，而这个思维是check当中了，进来进来之后再把它顶歪了，指的分支底下我们点击它下面有一个可瑞塔，你有分支可瑞塔有分支f一a这有I还有长在他我们创建分支新的分支名字称为非油车杠，它是什么短信对吧？这个系统的页面我们在系统这个页面当中，将来所包含的所有的页面，我们统一都把它放到分支底下来进行图像，那完成了之后点击ok。好现在的话我们的分支创建好了，并且我们现在已经切换到备用者CS配置文章，接下来我们所添加的一些代码等等，它都在随时配他们对象，不会找不到顶端的数据对象对吧？Ok那么这个在给他做完了之后，那么当然我们也能够判断谷歌的分支底下的一些这一些，但因为我们刚才暂停一般的分底下给他创建了构成了一个ok回到我们的QQ器可能有打通，回来了之后大家注意看，现在我们上面这个问题就是分支状态已经发生了改变，对吧？比如说我们现在的代码的发展对应着c对比一下之前这波人还是非油车 Start UI，不知道大家还有没有印象，你可以翻一下我们的视频。好，我们下一组按照我们的神仙步骤，他第一部分就是说让我们去设计添加一个Q7的实名制界面，从此我的命y里头对吧？然后把它的宽高修改成1350792。好，选中b出来，然后给他来上添加新文件，选择t给一个带内的就是UI文件，然后往下走。下一步把这个类的名称我看一下，给的是我的命关系。Ad安恒w ID第一期我的命管理好，下一步然后对吧点击完成。这样子的话我们的UI界面就把它做好了，并好了之后人家说我们给澳门的宽高修改成1350792，ok这回就把它修改成1350792792。好13507921350792没问题，然后按照以下的方式来进行布局，那么先通过 q外地才能到耳朵里外的当中，把他的摘抄内容修改为大家都格外的问题。来先给他拖拽是吧？500米进来，拖拽完了之后右键单击，顺便把它的背景颜色去修改一下，这个是b1CK我们有本地单不让的杠co等他了，ok颜色我们直接在这里它来进行选择就可以了，选择一个就是你自己喜欢的颜色，完了之后确定再往下来走的话，我们站到内蒙修改一般不让到对放在外头不让到外礼堂，这不知道是第一CK，这都有稳定到，当然格朗达w ID这一切我们都要单独浪到外地，他应该是想要让这个问题可能冲淡背景对吧？大家渴望的问题颜色涉及到，然后点击我的命，水平部就将它的拉丁水平部就要将它的拉丁和史密斯全部设置为0，奥迪密然后水平不实对吧？水平不全，完了之后选中将右侧的属性再往下拖，把我们的所有的这些挖井和CC一样给它全部设置成0，这样子的话，我们的背景就把整个的页面给它填充了，对吧？然后他说再往上走，让我们拖拽1个QQ ID通道，8个浪子当中我们加1个内容的发明为卡可以的，我们先给他拖拽一波进来拖了一个外部条件，这个能不能把它修改为contarner ok吗？Contar等于2， co等于tar可以没问题，然后把它的背景颜色在去修改一下。一一CK这样，第八个格朗得到co好了。好，然后它添加上一个颜色，比方说就改成这种灰色了，分好了添加上确定，完成了之后接下来再往下走，应该就要布局了，对吧？他说点击这个背景水平布局完了之后，完了之后就是将大格浪的高等设置位设置为5，垃圾设置为是垃圾设置为不是包头模拟设施的包塔设置为5，其余8.5它设置为使用，此BC为0设置为0。好，完了之后先选一个大家都忘掉，然后点击上一个就是水平布局。好再让他往下来走，他刚才说的是将包腿对吧？甚至578甚至为16，左是16，顶是16，又是16包，奥特他给的是5CC，刚才这个设置的是0对吧？0，ok这个是我们在里面给到中间的这一层对吧？然后它再往下走，我们拖着一个天空浮影和外地通道考生的当中上下摆放 QQ福利，我们在上外地投，在下然后将我们在做内容名字给他修改BT m box以及配置box，ok我们先给他拖拽一个q福利，再拖着一个q管理上下摆放福利，我们再上对吧？注意你不要看这个位置上下的话，可能不是我们真正框架的上下。好，我们先改他的，我们内部上面是我们的b7n box，底下是我们给他配一支boss平台之夜，a之boss，然后把它的背景颜色顺便在这边去修改一下，eckgroud杠Col好把拿去复制一下，完了之后我们给它添加上一个比方说是这种是紫色的红色的，好，底下是我们的杯之boss。好，再添加一个，比方说给他添加一个就是蓝色的确定对吧？再往下来走，我们再来看。然后让我们选中康菲特，点击垂直布局，将康菲特的靠谱和包头设置为20赖普瑞设置为24，先选中然后垂直布局对吧？完了之后往下拉，他说的是上下上下是20左右是24，左是24，右也是24，上是20，下也是20对吧？也是20。再往下走space一样给他设置的是15， Space设置的是15，好space甚至15完成了之后，将BT box的最小和最大的高设置成我先把你选中，往上滑最大最小的高度，把这个高度给成60，高度60对吧？60这样子的话，加上就是我们的文本，我们将来给他办理。好完了之后第四个步骤，他说通过这两个q18通道，并且boss当中水平摆放，我们再从内容从左往右依次修改为txt以及路1000。好，然后你方拖着两个按钮进来，这个是一个对吧？再给它拖拽上，然后按钮就左侧，这个它是我们的 check checkb贴对吧？然后右侧这个方法我们给他我点以前一个是审核管理产品，一个是页面角色管理对吧？然后他们就给从审核管理，好这地方是我们给到角色管理来回答回答。之后我们看到他说这可不行的文本修改为审核管理，入并签的文本修改决策管理没问题，然后再拖着一个水平弹簧放在右侧点击并填boss，然后选择水平布局对吧？将不能拉进来和space它全部设置为0。好，那么先给它通过这样一个水平弹簧进来，完了之后选中冰天boss点击水平布局对吧？完了之后选中冰天boss往下滑，他说将垃圾银行和死背景它全部设置成0，这样我们的按钮它就挤在一起了，对吧？挤在一起按钮中间感觉还有缝没关系。因为我们刚才确实把它的并签，boss实际上全部给它设置成了，对吧？思维体应该是不能挤在一块，他现在挤在一块，到时候再通过样式来和厨师去看一下。完成了之后，完成了之后，他说这太可和入冰千两个按钮的最小和最大的宽度均宽高均胜成100×600，宽高均胜乘100×600，ok先选择你，然后往上滑，把宽度设置成100×60是吧？100×60对100，高度在里面是60，宽度100高度60对吧？高度60这个是审核，还有我们的角色宽度100高度60宽度，上面就有一张宽度100高度在里面60，过程这样子的话，我们就将两个案子在这个里头的话就完了，没办法处理了，处理完了之后，然后他说再让我们拖拽两个KO y地方到卡片的当中，加上还是我的拆k以及如何推广，然后把它的背景颜色顺便去修改一下，这堂课我们先来通过绝对捕捉的这种方式来进行处理。Ok把它在这个就是放进来，放进来了之后，然后同志们注意看，我们先给他拖到这两个，外头先给他拖了一个，然后再给他拖拽上一个，那一个把它称之为我们的就是拆科推广checkjable好，另外一个把它称为肉推动 Ol，一对你想更好，role如tabero对吧？那给完了之后顺便把背景颜色放在这个位置给他去修改一下，把它改成粉色的中文的符号对吧？底下这个是我们给到，然后把它和辐射决定，所以这次我们就不能给它进行什么水平垂直布局了，水平垂直布局了，它就把那个页面平分了，对吧？那么你给它来上个物理布局，然后它的大小和我们的主界面变成一样大的话，我看一下它的量是多大。1270656。好，那就改成一个00对吧？从左上角了。然后这个是1270656，112706656对吧？然后把它选中了之后也是001270656，这样子的话将来我们在点击的时候，让它底下这两个外力层就是给它竖开整数开整之间来进行切换就可以了，对吧？具体主要看你点击的是哪一个按钮，这样子的话我们的主界面就把它布局完成了，给同时思考一下怎么选。接下来我们将系统页面的样式添加上去，在添加之前先将之前我们添加的测试颜色给它全部弥补掉。好，从上往下，然后将qy里头把它添加了颜色，全部拿掉，确定肯定的颜色有拿掉。然后再往下走，就是我们的q福利就是勘察颜色也有，他说拿掉对吧？拿掉，ok这部分还有冰天boss拿掉，还有我们底下给他拆壳，以及若退保这两个外置的底下，它也是有颜色的，把这些颜色通通的给它拿掉对吧？拿掉。好，那拿掉了之后，然后我们再给它从上往下来进行添加，首先就是我们的背景，这个是backbackgroU盘，第八个隔乱的wid这一期管理bck ground杠Col当然可能都擦了，这个背景的颜色，它在里面给的是 f 5f6、f8对吧？F5f5f8点击应用。让我看一下bcbackground backgroun啥少了一个对吧？找了一个地方，就说压缩点了之后它没有生效，好，再把它打开点击一个应用对吧？那么这个背景颜色它就上去了，完了之后再往下走，我们给康定了，给他去添加上一个颜色，这个是井号 co ntar你好好听懂。 Container dackgr不用了，in，container guns，corcolr hello。它的颜色在里面是白色的123456，而且它还有圆角的效果，be o b包得杠adms对吧？包括瑞是把圆角半径给它设置成1010个像素对吧？10像素就是我们这个位置的角标。好，再往下走，就是我们的btn btn boss井号，这个是btn boss，首先就是burb包的它的边框，我们不要将包的发出来，发出来完了之后边线我们重新给它来进行设置利用偶尔 boa一包的给一个包括Bot，比如说它边框底下的线对吧？给成两个像素，然后 solidsolid实际上颜色改成号井号f5f56f8f5f6f8，完了之后点击一个就是应用对吧？好，这个标签设置设置完了之后，就是我们的 b泰克b签，也就说这个按钮对吧？按钮今后 check代购并且审核的这个案子，这个按钮bckground杠Col按钮的颜色的特点是井号12345，是的对吧？白色。然后他的字体光泽范围到FM安排管道的范围之前，我们就把它给微软养黑，微软养黑，布朗科赛自己大小大小龙把它改成14个销售对吧？24个星座。然后就是Col自己的颜色，完了我们把它改成井号，这个是三EC ecefe三、ecefeff好，这个是文字的颜色，再让它往下走，包的将这个按钮的边框给它拿掉，对吧？把它拿掉。好，然后我们顺便再将按钮上的文字就是加速一下，就是当哥刚刚一台电器bom和他对吧？对，完了之后点击一个就是应用对吧？应用好再让它往下来走往下走，往下来走我们看一下底下我们的一个如何推广对吧？刚才设置的是check，现在我们把如何推广它去处理一下，来进行了警号85，然后一入可以结合。首先第一次KTV有 D叫cl，当然口罩的它的背景颜色也是 ff，对吧？白色的，这样它往下面走了，首先得给它翻和杠size，它不能塞，发生在的就是字体的大小对吧？自大变成14个小时，然后你的自己的方格杠am包括上面自强变成未来要黑为亚黑对吧？亚黑。好，那完了之后，字体的颜色 Col大量的把它改成井号，6再让他往下来走，打包打地瓜一样，包的杠Bo TT o还是包的包？包的包头把它改成两个销售对吧？两个小时还是还有哪里时间到？然后这个颜色是f5、f5、f6、f58、f56、f5、f6、f8。好，这样的话往下来走这个位置它就利益的话包子把它拿掉了，来让我们点击一个就是变动，我看一下我们这个位置点错确定应该是给差个b签来进行社交。我看一下入比性的你要进来应用。好，刚才我们的 check标签当中有一个按样品应该是错了，边框它确实是没有的，它这里面也有地包的，有理解完了包的包头包头包的，包头把它给了两个销售也是实心的对吧？S货还有s然后颜色在里面，和我们文字的颜色是一样的，他们先考虑一下，然后把它给关了，然后加上应用，它底下有个项目出来对吧？他说完成之后回到我们的主界面来，也就是bat plan，别的不来龙来看一下，应该是在这个地方对吧？找到我们的信，我就将上面的 qd版的发布材料看看它是我们的系统文件，然后把它内存提升一下对吧？提升一下。这种方式提升为类名，看到我们自己在这里参加，刚才我们把它表示能力AP传达我的命y里头w ID第一届添加对吧？然后使用提升这样子它的类型也就提升过来了，提升完成了之后，让我们的程序运行起来，我们看一下它运行起来的效果之后，点击了系统的页面切换按钮之后，那么要在窗体的位置对吧？要能够看到我们刚才所添加的就是管理员的系统管理的页面。好现在我们的程序还正在编译，编译完成之后，程序栏它现在已经运行起来了对吧？点击我的到我的页面，然后点击系统那就切换到系统的页面了，对吧？切换到系统的页面。现在在我们的页面当中只有两个按钮对吧？只有两个按钮，这两个按钮到时候我们把从函数给它一绑定，然后底下有个页面你就可以让它去进行切换。
