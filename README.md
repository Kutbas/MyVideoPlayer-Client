...
老师课上的讲话：“
视频上传页面还差最后的一个小模块，就是修改视频的封面图，视频封面图它默认情况下为视频的首帧，但是如果用户对于首帧不满意的情况下，它可以通过点击更改封面图的一个按钮，对吧？这个它是个按钮，点击了之后，同样也会弹出打开文件对话框，让用户在本地磁盘当中选择自己喜欢的图片作为视频的封面图。Ok，接下来我们把这个位置的逻辑把它去处理一下。好。回到kog的位置当中，那么首先看一下刚才那个按钮的结合的内容它是什么？把它选中了之后，这个按钮它是8对吧？称之8层，回到我们的头文件当中，那么先给它添加上一个槽函数，这一次是那位的。On，Chand乘之b由这些o的button clscked collect对吧？让我们从函数上出发来光标放上来，奥特加回车，在原文件当中就有了生存方法的定义，同样我们在工作方法的内容把信号槽卡绑定起来，ok这个位置它就不是卡梅卡比起来了，是我们刚才给到陈抓的对吧？陈巴斯这个槽还是什么？也是我们刚刚所添加的分之8层，克莱克斯。Ok用户点击了之后，按照我们刚才所说的第一件事情就是我们刚用户点击了之后，按照我们刚才所说的第一件事情就是你选择一张图片打开文件对话框对吧？打开文件对话框，然后打开文件对话框，我们这个位置得需要警号，因刻录到提供大家带来的把这个费用带了过来，它包含过来了之后，比方说这个是q对吧？Q死症是我们给的CIO内部filecio它包含进来，完了之后在 Qcio f f一f带l的当中，它有一个静态的方法叫做get open file令对吧？打开文件对方ok第一个参数是他的副对象对吧？这个位置我们不用设置了，nll7加不点了，然后再往后走到它的第二个参数。第二个参数就是当前这个视频打开对话框，它的标题对吧？标题让我们给把它点头，选择视频封面图再往下来走，就是它的默认的目录对吧？不是默认目录我们就不给了。好，再往后走就是过滤串，因为我们现在选择的是图片对吧？选择的是图片，mages然后把括号给关，我们要选择你选你选中的目录底下的人图片，所有的图片它都能够显示出来，对吧？Png完了之后星号也是对吧？假设我们就他给这两种类型的图片，给完了之后分好了添加成本这样子的话，你选只要你选择了，将来我们就能拿到图片的路径，对吧？这个路径如果 File name它点的a怎么安排等于空，如果它是空的，那就说明用户取消了选择，那这个位置我们就给它来上一个打印信息。也就是说用户在取消了封面图的选择，对吧？底下就直接认可了，既然你已经取消了，再让他往下来走，底下不能说明这个封面图它没有任何问题，对吧？问题只不过我们现在拿到的是封面图的路径，既然拿到是剖面图的路径，我们把它变成一张像素图，也就是pxo map对吧？ Pxo这个px可能它内部有一个工作方法，就是你给一个路径给一个路径，我们能够将当前你所提供路径对应的图片把它添加到我们的程序当中来，这个是怎么给它范围内添加进来，添加进来了之后我们直接如果就是UI UI咱们的封面图我看一下，应该是他们利宝他们立不了，如果我们直接给他塞特批客座，对吧？塞特匹克所将这个图像给它设置进去，我们选择的图片它可能和界面的 qd表的大小不合适，对吧？大小不合适。所以在你添加之前，这个位置我们可以把图片给它进行一个就是伸缩变形对吧？让它和我们的界面的大小给它变成一样。这个就是PK搜MAC，完了之后让它往下来走，就是PK搜MAC，它内部有一个方法，这个方法是死掉的对吧？死掉的方法我们之前已经用过了。好，然后你要伸缩的话要伸缩要伸伸缩，你要把图片给它伸缩到多大，它的第一个就是它有多个存在的版本，你可以采用赛德那个版本，也可以采用宽高的版本对吧？宽高版本这个味道是我们的a max level，a match level，然后我们就把它改成一个就是 Size，把它的 size拿到了之后再往下走，它的宽高比对吧？这部分我们忽略它的宽高比的一个 Dig no xxx和Visio对吧？或者宽告诉你，然后让他以这种比较光滑的方式替我们来提供一个高质量的图，对吧？斯莫斯蠢子医生我没什么这样子的话，我们就把这个图像给它伸缩了，对吧？伸缩。我还是看一下图片他们要放到哪个控件上，这个位置找到我们的界面 UI图对吧？这个位置它是一脉之内部一般的力度，完了之后对于我们的法律的没有配置，把它换成里面是内部设置对吧？刚才曹函数我们在这个里面他已经添加了对吧？添加了。好，那么这些给完了之后，让我们的程序运行起来看一下，好，现在能否把图片给它设置上去？程序上它正在编译，变音速度确实是有一些慢慢的马上可以它变音速度这么慢，那么后续写完了就是写完了这该怎么处理，你们后续的代码的话肯定是越来越多。好程序运行起来了之后对吧？点击就是上传视频，然后让它往回退，再往回退一步，选择videos，视频打开，这个页面对话框显示出来之后，然后点击更改封面图对吧？确实弹出来了，然后他往回退e max，然后这个是上传视频对吧？那么随便的给他选择上一张图，比方说我选择风景图对吧？打开之后然后发现现在视频它的封面图上就已经被改变了，对吧？已经被改掉。Ok这个是我们在里面给他上传视频封面的上传视频的页面，这个页面当中我们已经将自己能够处理到的一些逻辑上给它处理完了，然后后续我们没有处理到，比方说添加这个分类对吧？根据分类到你去选择标签，并且将当前页面里面的这些信息给它上传到服务器的这些操作，我们就只能把它放到后续再来进行处理了。”