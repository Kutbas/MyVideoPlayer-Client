...
老师课上的讲话：“
接下来我们再看一下修改按钮的背后处理逻辑，回到我们之前写好的播放平台当中，点击我的在弹出我的页面之后，如果用户对吧点击了修改了按钮，那就可以弹出一个新的对话框，在这个对话框当中，那么就可以我们就可以修改用户的密码以及昵称，好把它拿过来，同志们注意看把它拿过来，也就是说当用户点击修改按钮之后，内部表示当用户点击修改按钮之后，对吧，会弹出新的对话框，一个是对话风，让用户能够修改用户昵称和密码对吧？都修改用户昵称和密码，当然这个位置此处的用户指的是已经登录了对吧？用户然后临时用户是不支持的，如果你下面你是临时用户，你想要修改账号密码，我没有办法能做到对吧？你先得要登录才可以进行处理才可以进行修改。关于当前我们系统当中的什么临时用户已经登录的用户，这一点我们后续再给大家来进行介绍。好，也就是说你点击了之后，将来我们会看到这样子的一个页面对吧？会看到这样的一个页面，在它的右侧这个位置上是手机号，登录用户的手机号对吧？也就说你现在当前用户他是谁登录的，你登录了之后我就知道你的手机号，我们会把他的手机号拿过来，显示在修改个人资料的一个页面，对吧？显示这个页面，当然你假设用户的手机号码在里面是180对吧？12345678，我们在显示的时候肯定不会把它全部显示，结果下面显示的是1234将中间的思路给它隐藏掉对吧？隐藏掉了。你可以看一下，正常情况下确实应该把它隐藏掉，我们看到之前我们给的工具的话，好像没有隐藏的话，确实有一点感到不太合理对吧？合理我们后期咱们再进行处理的时候，我们可以把它首先把手机号给它拿过来，对吧？拿过来之后再把它给裁掉。好，再往下来走，也就是说接下来那么就需要给他再去添加一个设计师界面了，对吧？因为设计师界面添加比较简单，关键是内部的这一些布局怎么一步一步的给它进行处理。那么看到这个页面当中它的元素其实非常的简单对吧？简单实际上刚开始规章它就是一个q利宝对吧？再往下走这个地方它也是一个QQ，它的每一行实际上都是q利伯郎加上一个谁加上一个就是q外地牌再往下走，底下是两个按钮，整体上是一个上下布局的结构，对吧？整体是一个上下布局的结构，比如说两两它们两个就是一个 QQ是8层对吧？ Qq是8层，然后慢慢所给到手机号，账号密码，然后我们给的昵称对吧？给的昵称它实际上就是一个QQ，用来想这一些说明符号的q q按照aabe还有它是一个福利宝，然后我们给到这个位置，他们两个实际上都是编辑框对吧？都是编辑框， l APP完了不对？兰阿兰、DD、h三n力，但是大家要稍微注意一下的是在它的内部有一个按钮，这个它就是没有一个按钮，点击该按钮的话都可以修改用户的密码对吧？修改用户的密码。然后我们可以看一下，然后先点击修改函数对话框当中，我点击对吧？它会弹出一个新的对话框，然后我们在新的对话框当中你去处理用户的账号和是设置它的新密码，设置完了之后点击个提交，然后把密码就设置了，完了之后你再去设置用户的昵称对吧？用户的昵称。好，这个是咱们给的整个修改个人资料，它页面的布局整体上看起来还是简单的对吧？所以说有关比较简单的页面和将来我们在布局的时候，就像咱们采用的是这个绝对布局的方式来设置文档。对之前咱们进行布局的时候，基本上采用的都是向内布局，向内布局就是说我先把界面给你分成几个部分对吧？然后在它上面拖一个外地图，外地图放进去之后，然后再把咱们的框架放在我对这个外置方进行一个什么水平布局，垂直布局、栅格布局等等，相当于把咱们最终的元素是放到布局器里面去了，对吧？放在布局器当中，将来我们的页面进行了调整之后，或者说控件的宽度高度进行了调整之后，其他的控件它会自动的来进行调整，自动来进行调整。对于一些简单的页，对一些其他的页面，或者说像这种你要进行什么重叠等等这些页面，我们讲或者说一些你需要更加精细的去控制它的位置，这种页面我们可以采用绝对的方式来进行布局，现在我们随着个人资料咱们就采用绝对的分工绝对布局的方式来进行页面的设置，大家也可以去体会一下对吧？两种页面布局方式它的差别。好，基于这一些布局的布置方，同样我已经把它就是放到我们的帮助文档当中，对吧？一步2步、3步、4步、5步、6步已经把它放上来了，大家下来了之后同理按照我们所给的步骤，从上往下逐步来进行操作就可以了，对吧？一步步的来进行操作，就可以把咱们的页面给它构造好。接下来我们先给他添加一个设计师界面，他说设计师界面就是把这个力的名称把它修改修两遍，logo底饭塞尔夫、戴尔罗格，至于澳门传统它修改为440334对吧？334。好回到我们的课堂当中，然后往上滑找到b ng l右键单击在弹出的页面的菜单当中选择添加新文件，这一次我们要添加一个kut，设计师的为界面对吧？前面我们在底下就选择coatee外地b然而foam Klus带类的一个UI文件，完了之后选择底下就是让他继续选择外力层对吧？完了之后下一步然后把它的名字在这个位置给它改成 modify mo选择tot weddings designer form class带内的一个UI文件，完了之后选择底下就是让他继续选择y对吧？完了之后下一步然后把它的名字在这个位置给它改成 Modify modadis修改对吧？ Modify mse sales dra还有一第二楼的对吧？我看一下被告是莫里卡姆第二款马赛尔夫戴尔，第二类，完了我这 d a他的我就这样 logo对吧？对了完了之后下一步点击完成，这样子的话，我们等于设计师界面就把它添加成功了之后，比方说把他的作者的宽高修改成440×334440×334，我可以选中，然后这个是440对吧？高度在里面为334334，好，接下来就按照我们刚才所给的这几个步骤，从上往下逐步来进行处理，就可以把他们的页面设置好。”