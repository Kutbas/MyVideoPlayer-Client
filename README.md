...
我的页面布局分析
当用户点击“我的”页面切换按钮时，就会显示我的页面。

仔细观察发现，我的页面整体属于上下结构布局，从上往下依次为：基本信息区、我的视频区、视频
信息显示区，每个视频信息框可以复用VideoBox。
当用户在播放视频页面点击上传视频的用户头像时，就会显示其他用户界面，这两个界面非常像

我的页面支持：修改信息、上传视频、删除视频、退出登录
关注页面支持：关注和取消关注
其余都是一样的，因此我的页面和关注页面可以当成一个页面布局，将来显示的时候，比如创建我的
页面，将关注按钮隐藏，其余部分均显示即可。
...
退出登录按钮
QPushButton
QPushButton，点击之后会弹出一个文件选择
让用户选择所要上传的视频，并将视频上传至服务器
视频上传成功后，会弹出视频信息的上传对话框



显示用户上传的所有视频———审核通过
点击删除按钮之后
每个视频信息显示在VideoBox中，注意此处的videoBox上的视频播放信息隐藏，删除按钮显示
会弹出一个删除菜单
用户点击删除菜单项时
所有的VideoBox都在QGridLayout的布局器中
会将该VideoBox对应的
视频信息从服务器上删除掉



在页面设计时，可以将我的页面和关注页面综合起来设置，在进行页面显示的时候，如果是
我的页面：支持修改用户图像、支持修改用户昵称、支持退出登录，支持上传视频、支持删除视频
关注页面：不支持修改用户图像、不支持修改用户昵称、不支持退出登录，不支持上传视频、不支持删除视频

”

老师课上的讲话：“
视频播放页面处理完成之后，接下来我们看一下我的页面的布局处理。回到我们的设计图当中，我们知道当用户点击主页当中左侧的我的页面切换按钮的时候会显示我的页面，所以我的页面指的就是当前登录用户他的一些个人信息，以及之前所上传的所有的已经审核通过的这一些视频。Ok，那么截个图我们先把这个页面拿过来看，大家进行一个简单的说明。好，放到我们的画图板当中，这个页面当中它的元素其实也挺多的。好，我们从上往下从左往右，然后依次给大家说明一下。好，来走到这个位置，它是显示用户头像对吧？它在里面是显示用户图像，当然它不单纯的是显示用户图像，它实际上也实际是一个按钮，当用户点击的时候，它也可以修改用户头像，它也是可以修改用户头像的，对吧？这个方式我们在里面给到最左侧更改用户头像这个按钮，当然这个按钮上它含有一个遮罩层，按钮是圆形的，是圆形的，不是按钮是圆形的，就是我们的头像是圆形的，对吧？并且这个图像上有一个灰色的，有一个灰，这个是灰色的遮罩层对吧？有一个灰色的遮罩层，那么现在通过他给我们提供的设计图稿就可以看得到，对吧？好，再往右侧走廊这个地方，它显示的是用户昵称，用用户昵称的话，其实上就是一个qladel对吧？就是一个q利宝，然后再往后走这个位置它实际上也是个按钮，按钮就是我们所给的q58层按钮，这个按钮点击之后可以修改用户昵称和密码，我们用户在登录的时候，他可能是用户昵称和密码进行登录对吧？然后点击一下就可以修改用户的昵称以及修改用户的密码，好再往右侧来走这个位置。这个位置它就是当前用户的一些个人的数据信息了，对吧？个人用户的数据信息，比方说关注数用户他都关注了谁，然后粉丝数就是谁关注的当前登录用户获赞数当前用户他上传的所有视频总的一个获赞数对吧？播放数也就是他上传所有的视频总的一个播放数。好，所有的这些空间它在哪都是一个QQ对吧？ Qq，而且这一些QQ它将来放到放在一个山哥不服气的货，那么看起来它就是一个二维表格，对吧？二维表格类似的这种方式来进行布局的，它将来也是在一个上课不去记当中，好，再往右侧走，故障，它是一个登录按钮对吧？不是是一个退出，再加上退出登录按钮，它也是一个q出师对吧？可以不是吧？Ok再往下来走。我们所看到的右侧的这个这个部分这个位置，它其实就是一个说明，就是说明信息对吧？因为现在是我的我的页面，此处显示的是我的对吧？待会就是这个位置还得需要给大家再来进行一个简单的，既然说明信息它也是个qla be，然后也是一个QQ对吧？个QQ再往右侧来走，还有它在里面所给的一个就是上传视频，这个上传视频它实际上也是一个按钮，q50b有机器人对吧？ Q58寸点击之后会弹出，上传是会上传，会弹出就是一个文件选择框对吧？让用户选择所要上传的视频，并将视频给它上传至服务器对吧？上视频上传成功后会弹出一个视频信息的上传对话框对吧？上传视频信息的一个对话框，ok在这个位置上也就是说给大家在右侧看一下。对，视频上传把它就是往大里面放一点，对，就会给我们显示这样子的对吧？像这样的一个窗口，那么在这个窗口当中可以去编辑视频的一些信息，比方说视频的封面图，然后视频的标题，视频的简介，视频的分类，视频当中的这一些标签等等，这些信息将来我们可以会可能会按照分类和标签来进行搜索，对吧？包括这些什么标题简介视频封面，它都在视频显示的时候就需要用到，也就是在 Video box当中所显示的内容。好，再往下来走，同志们注意看整个它底下这一部分，这一部分它实际上是显示用户上传的所有视频，而不显示用户上传的上传的所有的视频，当然这个位置的所有视频，我们当时在处理的时候把它处理程序是审核通过的所有视频，其实把发行人审核通过的所有视频在这个地方也是有一点不合理的，当然大家下来之后你可以对这个味道进行一个你自己将来在实现的时候可以对它进行一个简单的处理，对吧？比如说你可以让它显示用户上传的所有视频对吧？把这件事在这个视频上，你将来可以添加上一个标签，哪些视频是审核通过的，哪些视频它是没有审核的，哪些视频是对吧？已经被下架了，哪些视频是它上传了之后被打回来的，也就是被驳回了。我们可以在 Video box上再给它去添加上一个就是QQ微博来进行说明，对吧？好，我们在做的时候这一块就简单了一点，就直接显示已经审核通过的视频了，那么他那帮我们看到这些视频也是以行列的这种方式，也就是说类似于表格的方式来进行处理，对吧？我想每个事情信息都是一个vivideov6box，对吧？每个视频信息一个位置信息都显示在 video boss当中，也就说它和我们的首页当中实际上是一样的，只不过比只不过和首页当中比的话，媒体boss当中它没有显示视频的播放数据信息以及对吧？没有删除按钮。好，注意此处的全部的vivo video box奥克斯它的上的视频播放信息隐藏了，对吧？然后我们的删除按钮了，让它删除按钮让它显示出来了。好，删除按钮的话，在这个位置它不是说你点击三个点的删除按钮之后，它就立马删除了，在这个位置它就是删除按钮之后，ok点击点击删除按钮之后，对吧？会弹出个菜单，我们这个用户点会弹出一个删除菜单，让用户点击删除菜单项时，会将该跟video box对应的视频信息从服务器上掉对吧？删除掉。好，这个是删除，按它在它的功能。好，我们刚刚说过所有的这些事情它都放在video，每个事情它都用video box来进行描述，对吧？所有的所有的 Video box它都在这个过程都在都在就是一个q IQ格的料子的杜绝气氛对吧？不去激荡大众。当然用户将来上传的事情可能会比较多，一个页面将来显示对吧？显示不下，我们看到在它右侧这个位置的时候还出现了一个垂直滚动条了，对吧？将来我们的 Q这样 ID q可是它内的它处在一个就是q SCI拉到64个人2楼当中，这样子的话你的视频多，我们左侧的滚动条它就对吧显示出来了。这个是我们的我的页面。而除了这个页面之外，还有一个点大家得需要注意，当我们用户在播放某个视频时，如果对当前视频非常感兴趣，他想要想要看一下视频上传的用户是否含上传过同类型的其他视频，也就是说他想要对吧？看上传者的个人信息，或者说或者说其他事情的时候，可以通过点击对吧？可以通过点击播放页面的用户图像也能够跳转到视频上传用户的页面，什么意思？大家注意看，还是在我们的投稿当中给大家来进行说明，比方说现在我正在播放视频了对吧了，用户他上传的视频特别有意思，我想要了解一下当前的用户他还有没有上传过其他特别好看的视频对吧？包括我可能想要看一下当前用户，他的都关注视频播放情况以及关注和被关注的一些数据等等，我们就可以点击用户头像，当他点击用户头像的时候，它也会跳转到预设我的页面，当然这一张我们不把它称之为我的页面了，它在里面是一个关注页面，我可能想要关注一下用户对吧？关注一下当前视频上传者的用户，好，所以将来我们也有一个优势，关注的页面，这个页面大家仔细观察，它其实和视频上传信息的页和我的页面非常的像，对吧？上面这个是我的页面，然后底下这个位置它在里面关注页面对吧？我想要关注一下当前用户，或者说我之前关注我现在我不想关注它了，我想要取消关注取消关注，那都是当前我们所看到的这个页面。这个这个页面它其实和我的页面当中的布局基本上是类似的，对吧？布局基本上是类似的，上来之后同样也是用户图像对吧？然后这个是用户昵称，但不一样的地方是大家注意不一样的地方是不能修改用户图像以及用户昵称，对吧？这些因为你又不是当前你要修改用户头像和昵称的时候，你可能只能修改你自己，你不能去修改别人的对吧？去修改别人的。好，然后这样往右侧走，这个位置它其实都是一样的，就是相同与我的页面相同对吧？相同，然后再往后走到这个位置，它稍微有点不一样了，在我的页面当中这个位置它是删除我退出登录按钮，我的页面中是退出登录按钮对吧？然后再关注页面是已关注，或者说它是也关注对吧？关注的一个区域。已经关注了，或者说我现在没有关注它，它文按钮上它显示的就是关注对吧？关注，然后再往下走，注意这个位置上也有一个区别，对吧？也有一个区别就是我的页面，我的页面此处是有一个上传视频的对吧？按钮的，然后关注页面此处没有关注页面，它在里面是没有的，对吧？然后再往右侧这个位置，它显示的文本一点稍微有点区别。如果是我我的页面的话，此处显示的是我的视频对吧？然后在关注页面，因为现在你是点击了，其实是传上在那个视频播放页面，点击了用户头像之后，我想要看一下当前用户所上传的一些其他视频的信息量，所以说这个位置它显示的该处显示的是他的视频，因为我也不知道，因为他是男的还是女的，所以说只能是他的视频，对吧？他的视频。然后再往下走，底下这一部分他其实基本上也是类似的对吧？基本上类似所有视频他都在或者说每个事情的信息，它这里面是由 Vr dov丢boss在里面进行说了对吧？进行描述我们所有的这不是所有的，因为ID有video box，它都处在q这ID q格瑞特料子当中，当然这个就是q格瑞德，利奥特朗它又处在我们的QS里，对吧？ Square当中，当然在 vo box在显示的时候注意这个位置它没有删除按钮，每每个video box当中它没有显示删除按钮，我的页面中的 video box上显示删除按钮，然后在关注页面中的 viduvu box上删除按钮就是隐藏的对吧？那么隐藏删除隐藏视频的删除按钮，因为视频根本就不是你上传的，所以说你也没有你在这个地方也没有能力去删除，也不敢让你去删除对吧？不敢让你去删除，所以说我的页面它这个文章是没有的，也就说总体来说我的页面和关注页面它们两个是类似的，对吧？ List所以说将来我们在页面设置的时候，我就把我的页面和关注页面的给它结合起来，对吧？将来你谁用，我把你页面上需要显示的给你显示出来，不需要显示的我们把它对吧隐藏起来就可以了。也就是说我们在页面设计时，我们可以将可以将这个我们可以将我的页面和这个是关注页面对吧？综合起来设置对吧？在进行页面显示的时候，如果是没办法，如果是我的页面在我的页面的话，那么让它去支持修改用户图像，然后支持修改用户昵称对吧？然后还支持在这个地方支持退出登录，然后支持上发视频对吧？上传视频支持删除视频就可以了，如果那是关注页面的话，上面的这一些支持我都让他不支持就可以了，对吧？不能支持修改用户图像，不支持他拷贝者 C错v对吧？在这边统一让它不支持就可以了。在这个页面显示上，我们稍微也让显示的时候就处理好，稍微做一些调整就可以了，对，稍微做一些调整。Ok这个的话让我们给到关注的页面和我的页面对吧？我的页面当然上面我们刚刚所说的这些，只是我们页面当中都有哪一些元素，对吧？都有哪些元素，然后我的页面和关注页面他们两个页面当中有哪些异同点，异同点，接下来我们还可以再把页面它整个的布局给大家进行一个简单的说明来。再往下来走，同志们你注意看按照我们刚才的分析，在整个的就是我的页面当中或者说关注页面当中，它实际上就是一个上下布局的格式，对吧？上面这个位置上我可以给它放上一个外地层，而这个位置它实际上也是一个外地层对吧？然后再往下走这一部分，它实际上也是一个外地层，能看来吗？你总体上是一个上下布局，把这三个外层放进来之后，给它来上个垂直布局，然后把它的拉低和高度它去设置一下就可以了，对吧？一下然后在我的内部的话，你算是一部分，你在里面算是一部分对吧？你是一部分，包括了我们右侧这个按钮，它也是一部分，你们总体上是一个水平布局，那么中间我可以给它放上一个弹簧，把你撑开都可以对吧？这里面都可以。好，然后再往下走，到达我的中间这个位置是他 Q利部，然后在你的右侧他是一个是一个上传识别的按钮，中间我也给你可以放一个弹簧的话重开对吧？重开然后再往下走。我们这个位置就是底下的视频显示区域，视频显示区域整个它实际上 q qscrorl斯诺安在斯科拉的内部，我给你套上一个q可能qgrieq格瑞德 out对吧？在 Lay out当中你再去显示添加就可能添加某v6boss就可以了，让他添加video，boss这样子的话，我们就对整个的我的页面它内部有多少元素，每一个空间它大概是什么样的作用，对吧？包括当前页面的布局就有了一个简单的了解。”