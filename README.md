...
// uploadvideopage.cpp 新增代码

UploadVideoPage::UploadVideoPage(QWidget *parent)中新增：
...
老师课上的讲话：“
在上传视频页面，用户他将来可能会修改视频的标题，以及给视频去添加描述信息对吧？来切换到我们的页面。如果用户将来输入标题了，以及添加了视频的简介了，我们右侧所给的它剩余的字数对吧？剩余的字数限制它也要随着用户的输入来进行动态的更新。这个逻辑的话还没有处理，如果要进行处理了，我们必须得要能够监控到。假设他们对于我们的客户安安理房，那么必须要能够监控到当前输入框当中的文本的变化情况，你只有监控到它的变化情况，才可以去更改我们右侧的输入文字，对吧？这个就是怎么来进行处理，我们知道在qt拉艾利当中，它有一个就是txt的信号，开拓，甚至信号指的就是我们内部的文本一旦发生了变化，我就给你发射一个信号对吧？它发射信号的时候，这个信号当中一个有个参数有个参考就是会将编辑框当中的文本请我们返回出来，来回到我们的阿布路的外地图当中，录到外地录到windows当中对吧？那么先给它绑定上一个信号，从函数去看一看。Ok这个方式 Word导航做一导手动挡，那里头对吧？ I里头找到三x和ch点点一定不是我们的拉麦德拉按照他一些文本发生了变化，ok这个参数它这里面是有一个参数的，这个参数就会将编辑框当中的内容对吧？就能反馈出来。此证我们给一个，因为他在给他文本能把操盘手给他填充过来，对吧？拿过来了之后顺便把槽还是给它一绑定，绑定好就是CNN和ect到那个头，然后在我们的UI界面当中有一个视频简介，我看一下您应该是开锁tfttle开头没有太维度探索，对，没有探索的话就是个q拉案例对吧？来这个是po那案例当中所发出的一个 Txt政治 x政治的一个信号，ok也是让当前页面当中的咱们刚刚所添加的 blue的配置里面有一个来埃里克txt的信号让他去进行处理对吧？那么在这个方法写的内容，咱们现在先给他打印一下，这个位置我们就直接把 face把它讲，但是今天打印了把我们的工具的文件包含进来，u加l变为吃保健，完成了之后让我们的程序运行起来，待会我们看一下它是否会发生改变就可以了。如果要能当我们在编辑框当中输入内容的时候，它能将内容实时的就能打印出来，那就说明我们就可以依靠开发车里的信号对吧？监控边框当中的一些文本的改变，他马上就编辑好了。程序运行起来之后同样点击我的，然后点击上传视频往回退对吧？退过来之后这部分选择的是video选中，打开主要就是这个位置的标题，猫对吧？你看猫两个猫猫对吧？只要我内部的这些文本发生了改变，减少一个对吧？我们刚刚所给的txt和称重的信号，它在那里出发了，对吧？既然已经能够捕捉到这个变化了，我们在这个方法当中所要做的事事情的话其实就非常简单了，我们只需要看一下你的咨询个数，对吧？你的支付个数，然后把这个个数和他的总个数在里面去拼接一下就可以了，和就是总个数拼接一下，当然总个数来可以看一下总个数它是在哪个位置，因为总个数就在我们的 World life当中，所以说我们先得要通过world life把它的对吧？总个数你好拿到。Ok来回到我们的代码的文件当中，基本上是把工作的v6，也就是说我们先要从我们刚才给到我的last都获取不是最大的出入字数对吧？把最大的输入字数给它，拿到了之后，你底下这个问题可能得需要检测。检测text中等字数是这个是字数它是否超过了我的对吧？有没有超过那完了，如果没有超过，我们将我们把它拼接起来。如果没有超过对吧？超过，那么将就是泰克斯的长度和w还有d差不多最大字数对吧？最大字数给它。情人节。好谢谢，当然这个问题简单一点，你也可以在这个类当中给它去定一个产业变量，因为产业变量去记录它的最大字数也行，对吧？这个位置我们就直接让他去进行获取 QS准，这个是我们的 w还有d我的 w文化d对吧？字剩余字数，剩余的可能他就对吧说没意思了。审讯字数，然后它的一个文本应该是我们给的UI w沃德莱克斯，然后找到他的涉及到他文本对吧？文本把这个文本在里面拿到了之后，然后这个问题的话，我们把它转化成一个我看一下，再往下来走的话，我们就在这个位置就得需要去提取，它里面的内容要提取的话，假设我们内部的内容现在是6对吧？斜杠86806斜-80的话，我们就不需要在 word life当中对吧？咱们先给他去找一下index of index of在里面找谁找？斜杠。找斜杠，那找到了之后，它就会将斜杠的位置替我们返回回来，对吧？拿到了斜杠的位置之后，拿到了斜杠的位置之后，让沃德莱特在此它内部有一个卖掉的方法，实际上那个指标当中是不是也有对吧？这个过程你看它第一个方法就是你从这个位置开始截取多少个，如果是复位的话就表明截取到它的末尾，ok我们将烂pose进去，一个一加上一就是把斜杠，你看它现在指向的是斜杠的位置，那么要从后面8的位置开始建，那加上1就是走到8的位置对吧？后面电子参数不传了，不传过来给的就是1，-1就表明现在我要截取到它的末尾了。你想想我要截取到它的末尾，ok它截取完了之后也是一个字符串，这一步找不到它字符串里边它有一个偷印的方法，那么就可以将这个字符串给它转化成一个整形的数字，对吧？转化成一个整形的数字 Int，这个是我们代表所给到 totalo通通放就是我们所里的通通看对吧？这个通通抗通在里面，总的字数在里面已经有了总字数，有了总字数有了之后，那么把它拿到了之后，然后同志们再来看，接下来我就得说检测，看一下你里面的字数，对吧？这个是 Int类型的，我们给一个这个是 word Top， word count就是记忆x7techs，然后他也是一个文本对吧？文本，那么直接看一下它的 size就可以了，怎么算点的？认识就是它的长度对吧？长度把这个长度拿到了之后，按照我们所给的这一块，就是我们需要去检测一下，看一下你现在所给到 word的抗潮，它如果大于我们所给它偷，totl偷偷康复，如果超过了的话，我就不能给你进行假设，对吧？如果小于等于小于等于我们再来进行假设，否则的话为什么其实我们什么都不用做了，对吧？或者说我们可以给他来上个打印，那个也就是说我们的视频标题对吧？视频不管它是视频的标题超过了我们的80个字，原本点的就是80个字，你如果没有超过的情况下，接下来我们在内部所要做的事情就是UI怎么的我的net对吧？我要重新把它的文本复制一下，这个文本大家注意看丢死症在我们的内部它有一个number的方法， Number的方法就是来进行这种数值转化成字符串，对吧？数据转怎么出来，那一个是我们给到我的状态，然后再往后走，我们给它拼接上一个斜杠对吧？斜杠再将我们的总字数给它拼接上去，总字数也是q4 Q4 MB对吧？叫不到total偷偷靠谱给它拼接起来就可以了。平行这样子的话，当我们在标题的文本框当中进行输入的时候，就是右侧的你输入字数它也会跟着来进行变化，对吧？也会跟着来进行变化。这个位置它给了一个提示说我们可以用 me的if来进行代谢，来进行代谢，我看一下这个位置可以干嘛，100分我们没有这个方法对吧？没有这个方法我们就加强不管，先我们看一下我们的就是功能它能实现对吧？来让程序上运行起来。为了测试的话就是我们等会你可以把我们刚刚在内容所添加的这些代码把它先停掉，对吧？停掉这样子的话，我们每次运行起来之后就不需要给他去加载视频了，你直接进行窗口的切换。它现在正在编译，编译成功了之后让程序运行起来，把程序运行起来，点击就是我的来到我的页面，完了之后上传视频对吧？上传视频往回退，然后点击windows内部的视频卡选中打开页面现在是已经切换过来了，然后我们还去输入标题，这个是视频对吧？你看它现在是22个字，然后12345你随便点，现在是8个了对吧？8个。我数一下12345678没错，我们看减少它右侧是不是也在发生改变了，对吧？也在发生改变，我们它给长一点可能会现在在里面是28个了，我在可能变还继续可能变对吧？对，你看他后面再给的时候就不变了。投票。然后现在是66 6 64、64个，123456789 10来给他添加过来对吧？74 8080最多就是80，如果超过80个字符的话，它基本上会给我们进行提示的，对吧？请提示我看一下，页面那个了还能不能够再输入进去，现在最后的字是少，我给他来上一条多，那么这个字它在里面还能够输入进去对吧？能够输入进去的话，这个肯定也不是我们所期望要的，而且期望要的在这个位置上超过了80个，我们把最后多的我们去截取一下，对吧？来看一下我们如何来进行设置，这个是test，同样我们通过need这个方法给它去截对吧？而且从d0的位置开始截，总共来给它截取80个就可以了。截取完了之后，我们将 UI再进一步的给到刚才是我的life对吧？把它的文本重新设置一下，设计一下就是三和txt才开始把追回来，回过来你错过我就重新截从左侧截对吧？零起始位置写，一直往后截取80个字符，反正你最多就是8080个字符。好，这个是咱们编辑框当中的处理方式。有了标题编辑框的处理方式之后，接下来我们再以类似的方式将简介编辑框去处理一下。也就是说处理剪切编辑框的时候，我们也得需要给它绑定一个开合称的信号，对吧？在信号并的层函数当中也要去当前输入的字数以及总字数，检测一下你的当前输入的字数它有没有超过总字数，超过了我们就截取，没有超过我们就进行拼接，大概也就是这样子的一个逻辑。Ok完了之后回到我们的文件当中，按F4接下来我们也给它去定一个侧函数对吧？这个方式可以的，然后这个空间它是plkl对吧？ Plan我看一下是plan，idea都一个编辑框他的开车趁着时候真的这个信号它没有参数，我们待会我们自己来给它去获取就可以了，对吧？来进行获取，ok完成了之后，F4回到原件当中，让我们把上面给它拷到c拷贝一下，拷贝这样接进来，ok会长，他就是我们的简介，这些bf我看一下简介编辑框不是这个是简介的编辑框，这个编辑框比如说你是个plan后来他知道a里头比如平安开始艾特就差对吧就好。好，那么这个味道也得需要把它变一下，比如普拉埃里克拜克，斯埃里克它内部也有个派斯特称的信号，我们点一下看到这个信号它现在是没有参数的，对吧？把它关掉，那完了之后基本上把那个槽函数的给它去定义一下， X槽函数我们刚才声明了，没有定义工商标给过来，按照奥特加回车定义，ok函数平方给他拷了一下，完了之后把这个就是方法给它替换过来，对吧？切换过来。切换过程在里面的实现方式，按照我们刚才所说的，你进来之后，我们这就是先要获取编辑框当中的文本，对吧？好，他先去获取uplam湖南开始I里头中等中等文本，这个文本大家注意看它都是q斯顿，而且还可以使用给一个在因为我们懂弗兰，但是爱你好对吧？他也有一个txt的，我看一下这可能是哭，不然txt不能太死了，方法对吧？这样子的话我们就将它文本拿到，把这个文本拿到了之后，我们再给他去定一个定义来说，我的放我要看一下你的字有多少个字，对吧？ Txt。第二个要论词，塞词和论词都是看它的长度，把这个长度给它达到对吧？长度达到了之后，接下来那么再给它去获取总的字数，获取最大字数，那这个最大字数，假设他现在在里面是能给到我已经输入了10个对吧？10个，后面是一个1000，这里面的处理方式其实和上面是一样的，我们也得需要先去或先去找到斜杠的位置，然后从斜杠之后截取到字母它的末尾对吧？这个老师给我指正。提供词涉及到剩余的老师，丰富gogl托福对吧？不能说这个偷偷看到他了，对我们先不要找其他方法硬的， Boss先找到我们斜杠的位置，斜杠位置我看一下人家找斜杠的位置就是字符串你对吧？把字符串先得拿到。Ok QS中这个是咱们给到那还不错，我再来谈结果，获得了 UI UI咱们那边有一个博瑞福莱德沃的对吧？来给我把他们拿到，我看这个类型应该是QQ对QQ把它 x给他拿到对吧？拿到了之后然后让 Word let你在里边去进行查找对吧？行 X你x就是看你的某一个字符的，所以能把这个写法同样给我。实际上所以他找到了之后，接下来让我们在这个位置再来进行截取，对吧？截取的话我的net点的一个密的对吧？我们从 Pose的位置加一的位置开始截取，一直截取到字符串的末尾，这样子的话我们就将1000的字符串达到，1000的字符串拿到了之后，我们通过它那边有个租赁的方法，就可以把它转化成一个整形的变量，对吧？就可以转化成一个整形的变量。Ok把它拿到了之后，底下我们再来检测，实际输入文本的长度是否超过总长度，对吧？是否超过一个最大长度。被告处这个位置同社用不到，我的抗体，我的抗体如果他在这个大于偷偷抗totalo亢，所以他这个字符串那么就让它内部偷印度的方法，将这个字符把它转化成一个整体数字，如果超过了对吧？这个大于的话就是通过如果它是小于等于小于等于的情况下，那就说明没有超过，那么没有超过我们在这个的话去进行空间就改为UI。Please let what say the test。Do you say the test？进来之后，首先我们需要用QC中内部它有一个方法对吧？ Number的方法。那我们得需要将我的 word count它转化成了字符串，然后拼接上一个斜杠对吧？拼接上一个斜杠。好，再往后走，将咱们刚才总的副总 Total photo count给它拼接上，拼接上去就可以了，对吧？否则的话那就说明你别超过了，按照我们刚才的说法了，我们在这个地方来进行截取就可以了，对吧？截取下一套平安that's right。Next要进行截取，就是有一个word状态，不要它完成力的对吧？你密道我们从哪个位置截，然后从0号位置开始环节，这个位置不是应该是text对吧？ Texg text me的0号从0号位置开始截，一直截取多少，刚好等于是咱们给他total偷偷抗塔，通常的点了一个通用it，对吧？来进行截取，少一个括号就行。这样子的话它内部这些内容始终都对吧，都不会超过我们到80，它能够给到1000个字符的话给到1000个字符好，让我们的程序运行起来，我们看一下它的效果，程序运行起来之后点击我的，然后上传视频往回退，往回退完了之后点击没有，选中视频打开，接下来我们在简介这个位置给它来进行输入对吧？输入你随便的再给它输入，比方说你好，我们就输入了两个字符对吧？两那么再来进行拷贝，你随便拿来进行拷贝，这个字符，它现在是以爱的方式来进行增加的，对吧？40，ok我们再来 C考虑c80对吧？80。Ok考虑c比方说160，考到c再来320对吧？320考到c再来这个640，640 640，我们再给他来上一个320应该是960对吧？960现在是960个字，那么再给他去进行服用。960 970 980993对吧？99 993 993的话，来再让他往后来走，我们看一下你好993，然后997对吧？997。我是一个当时输入到1000的时候，大家注意看刚刚才我们在输的时候，他现在是不是在最底下对吧？当我们输入超过1000的时候，你个那么发现这个滚动桥它是不是就滚上去了，对吧？滚上去了个确实没有输入进来，它滚动条滚上去的话，它并不是非常的友好，你输入不进来的话，我们也想要让滚动条让它在最底下，对吧？然后在最底下，ok，如果要让滚动条在这里，这样的话也就是你不要让它的光标往上移，滚动条它就走不上去，对吧？你看现在我们光标在这个位置，然后我输入一个个个，输入完了之后，光标它是跑到起始位置上，光标跑到起始位置滚动条它自然就滚动上去了，能看到吗？滚动桥自然就滚动上来了，因为我们这个位置是1000，应该是1000 1000，待会再来再去看一下，感觉到没有显示全应该是滚动桥的。滚动桥滚动下来之后，应该是把我们0给它挡住了，对吧？你应该是要走。Ok，这样的话往下来走，我们接下来所做的事情就是不要让滚动桥让它往下走，也就是说往上走，设置滚动条，一直设置滚动条，对吧？它一直在一直在，于是我们的底部底部要是甚至他一直在组织的话，我们首先你要去获取它的光标，非常的光标意外舞蹈出来，车开始爱你好对吧？它内部有一个方法就是开车，歌手歌手它就是光标的意思，反复的是一个q开始和哥嫂对吧？我们这个人qx dot c you are sor它就是我们的光标cau Sos国，是吧？把他们接受一下对吧？在 Txt的类的内部，它有一个木本身的方式，这个movie把光标的位置给它去改变一下，对吧？好，那么现在想把它移动到窗口的末尾，也就是说 tesg我们给的 cos对吧？它有一个end的这样子一个工具把它顶过来，end就是移动到末尾，那么在移动的时候不给它进行任何选中。然后这个是咳嗽咳嗽，这个是move把全都擦对吧？把全都擦，这样子把光标设置好了之后，别急，我们得需要将刚刚我们所进行的这一些光标的设置上，把它设置到文档，普兰泰安的房子对吧？塞托太多咳嗽 Txt，把螳螂就是这一个s码得过来。好这个位置这几个方法它大概的含义是这样的，首先第一个是我们给他开车咳嗽，开车咳嗽这个方法它是获取当前光标的位置，获取当前光标，这个是光标的位置对吧？这个位置。然后再往下面走了，木这个方法它就是移动光标的位置对吧？移动光标位置的时候，我们在那个地方提供了两个参数两个参数，提供的两个参数，这个参数它的含义移动到文本的末尾对吧？将光标将光标移动到我们文本的目录，好，还有我们给到第二的个的参数，这个参数它大概的含义就是我们在移动的过程当中不进行任何的选中来表示光标我们的光标对吧？在移动时光标的位置是固定的，不进行任何的或者说不会创建一个选区，什么意思？比方说我们刚刚开始光标在这个位置，我就把它移动到这一行的末尾，你移动的时候选区对吧？好，最后的这个方法把光标的给它应用到一个控件上，那么让程序运行起来，程序运行起来之后点击我的然后上传视频对吧？上传完了之后让它往回退往回退，选中我们的视频打开拉进来之后，我们主要测试一下就是简介的位置对吧？刚才我们已经给它进行了一个测试，你好，然后让他全去对吧？再来56个。64 72 80对吧？80~80的时候我们给他再来全选，没办法选择160，好讨论c这个老师320对吧？350c然后这个是640对吧？640 640，我们再给他来上一个320就变成了960了对吧？960960了，我们给他再去继续超。大家注意看这次超了之后你对吧？好，好，那么输不进去了，并且它的光标就不会移动到上面去了，对吧？不会移动到上面去了。好，这个是我们在里面给他提供的。再把它关掉，我们在里面给他提供的就是我们的简介，视频的简介以及视频标题在变化的时候，它右侧的输入已经输入的文字也会跟着动态的来进行改变。”